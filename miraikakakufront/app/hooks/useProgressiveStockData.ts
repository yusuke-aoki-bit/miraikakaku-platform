...
export function useProgressiveStockData(symbol: string) {
  ...
  const fetchStage1Data = useCallback(async (stockSymbol: string) => {
    ...
    try {
      ...
      let latestPrice: StockPrice | null = null;
      if (latestPriceData && latestPriceData.length > 0) {
        const lastEntry = latestPriceData[latestPriceData.length - 1];
        if (lastEntry) {
          latestPrice = lastEntry;
        }
      }

      const elapsed = performance.now() - startTime;
      if (process.env.NODE_ENV === 'development') {
        console.log(`Stage 1 data fetched in ${elapsed.toFixed(2)}ms`);
      }
      setState(prev => ({
        ...prev,
        details,
  ...
  const fetchStage2Data = useCallback(async (stockSymbol: string) => {
    ...
    try {
      ...
      const elapsed = performance.now() - startTime;
      if (process.env.NODE_ENV === 'development') {
        console.log(`Stage 2 data fetched in ${elapsed.toFixed(2)}ms`);
      }
      setState(prev => ({
        ...prev,
  ...
  const fetchStage3Data = useCallback(async (stockSymbol: string) => {
    ...
    try {
      ...
      const elapsed = performance.now() - startTime;
      if (process.env.NODE_ENV === 'development') {
        console.log(`Stage 3 data fetched in ${elapsed.toFixed(2)}ms`);
      }
      setState(prev => ({
        ...prev,
