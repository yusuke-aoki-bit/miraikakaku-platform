...
import { Plus, X, TrendingUp, GitCompare } from 'lucide-react';
...
export default function StockComparison({ primaryStock, className = '' }: StockComparisonProps) {
  ...
  const addComparisonStock = async (symbol: string, name?: string) => {
    ...
    try {
      ...
      const newStock: ComparisonStock = {
        ...
        color: COMPARISON_COLORS[(comparisonStocks.length + 1) % COMPARISON_COLORS.length]
      };
  ...
  // この関数は使用されなくなりました（API接続エラー時はデータを表示しません）
  const generateMockPriceData = (symbol: string): StockPrice[] => {
    return []; // 空配列を返す
  };
  ...
  const normalizeData = (data: StockPrice[]) => {
    ...
    return data.map(item => ({
      ...item
      normalized_price: ((item.close_price - firstPrice) / firstPrice) * 100
    })
  };
      ...
      {comparisonStocks.length > 0 && (
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[{ ...primaryStock, color: COMPARISON_COLORS[0] }, ...comparisonStocks].map((stock) => {
            ...
            const totalReturn = firstData ? ((latestData.close_price - firstData.close_price) / firstData.close_price) * 100 : 0;
              ...
              <div key={stock.symbol} className="border rounded-lg p-3" style={{
                ...
                <div className="text-sm space-y-1">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium" style={{ color: '#f1f1f1' }}>¥{latestData.close_price.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between">
