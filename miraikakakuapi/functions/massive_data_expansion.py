...
class MassiveDataExpander:
        ...
        try:
            ...
            try:
                ...
                if inserted > 100:  # 十分なデータがある場合のみ
                    pred_count = self.generate_extensive_predictions(
                        db, db_symbol, hist
                    )
    ...
    def generate_extensive_predictions(self, db, db_symbol, price_data):
        ...
        try:
            ...
            for period in momentum_periods:
                if len(prices) > period:
                    momentum[period] = (prices[-1] - prices[-period - 1]) / prices[
                        -period - 1
                    ]
            ...
            for days_ahead in range(1, 91):
                ...
                # A. トレンド継続成分
                if trend_signals:
                    trend_component = np.mean(list(trend_signals.values())) * min(
                        days_ahead * 0.05, 0.3
                    )
                ...
                if momentum:
                    mean_revert = (
                        -np.mean(list(momentum.values())) * 0.1 * np.sqrt(days_ahead)
                    )
                    prediction_components.append(mean_revert)

                # C. ボラティリティクラスター成分
                vol_cluster = (
                    vol_regime * np.random.normal(0, vol_short) * np.sqrt(days_ahead)
                )
                ...
                confidence = (
                    data_quality * volatility_penalty * time_penalty
                    + model_complexity_bonus
                ) * 0.9
    ...
    def update_progress_thread_safe(self, result):
        ...
        with self.progress_lock:
            ...
            if self.stats["processed"] % 25 == 0:
                ...
                logger.info(
                    f"進捗 {progress:.1f}%: 処理{self.stats['processed']}/{self.stats['total_symbols']} | "
    ...
    def execute_massive_expansion(self, target_symbols=1200):
        ...
        success_count = (
            self.stats["processed"] - self.stats["errors"] - self.stats["skipped"]
        )
        ...
        logger.info(
            f"📈 成功処理: {success_count}/{
                self.stats['total_symbols']} ({
                success_count / self.stats['total_symbols'] * 100:.1f}%)"
        )
...
if __name__ == "__main__":
    ...
    logger.info(
        f"✅ 大規模拡張完了 - 価格データ+{result['price_records']:,}件, 予測データ+{result['predictions']:,}件"
    )
