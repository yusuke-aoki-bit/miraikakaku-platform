...
class ComprehensiveBatchLoader:
    ...
    def generate_advanced_predictions(self, db, db_symbol, price_data):
        ...
        try:
            ...
            for days_ahead in range(1, 31):
                ...
                # 総合予測
                total_change = (
                    trend_component + mean_reversion + random_component + seasonal_adj
                )
    ...
    def update_progress(self, result):
        ...
        with self.progress_lock:
            ...
            if self.stats["processed"] % 10 == 0:
                ...
                logger.info(
                    f"進捗: {progress:.1f}% ({self.stats['processed']}/{self.stats['total_symbols']})"
                )
    ...
    def execute(self, max_symbols=None):
        ...
        logger.info(
            f"📈 処理銘柄: {self.stats['processed']}/{self.stats['total_symbols']}件"
        )
        logger.info(f"💾 価格データ: {self.stats['price_records']:,}件")
        logger.info(f"🔮 予測データ: {self.stats['predictions']:,}件")
        logger.info(f"❌ エラー数: {self.stats['errors']}件")
        logger.info(
            f"📊 成功率: {((self.stats['processed'] - self.stats['errors']) / self.stats['processed'] * 100):.1f}%"
        )
        logger.info("=" * 80)
