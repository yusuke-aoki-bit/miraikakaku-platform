...
class SyntheticDataBooster:
    ...
    def generate_realistic_price_series(self, symbol, days=1000, base_price=100):
        ...
        for i in range(days):
            ...
            daily_return = (
                drift + trend_cycle
            ) * dt + volatility * vol_cluster * np.sqrt(dt) * np.random.normal()
    ...
    def create_synthetic_symbol_data(self, symbol, target_records=2000):
        ...
        try:
            ...
            # 時系列生成
            prices = self.generate_realistic_price_series(
                symbol, target_records, base_price
            )
        ...
        for i, symbol in enumerate(all_symbols, 1):
            try:
                ...
                if i % 50 == 0:
                    ...
                    logger.info(
                        f"  累計: 価格+{total_stats['prices']:,}, 予測+{total_stats['predictions']:,}"
                    )
    ...
    def check_final_ml_score(self):
        ...
        try:
            ...
            logger.info(
                f"  銘柄多様性: {diversity_score:.1f}/25 ({price_stats[0]}銘柄)"
            )
