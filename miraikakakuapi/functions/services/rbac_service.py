...
class RBACService:
    ...
    def has_any_permission(self, user_role: str, required_permissions: List[Permission]) -> bool:
        """いずれかの権限を持っているかチェック"""
        return any(self.has_permission(user_role, perm) for perm in required_permissions)

    def has_all_permissions(self, user_role: str, required_permissions: List[Permission]) -> bool:
        """すべての権限を持っているかチェック"""
    ...
    def can_access_endpoint(self, user_role: str, endpoint: str, method: str) -> bool:
        ...
        endpoint_permissions = {
            ...
            ('GET', '/api/finance/stocks/{symbol}/predictions'): [Permission.READ_PREDICTIONS],
            ('POST', '/api/finance/stocks/{symbol}/predict'): [Permission.CREATE_PREDICTIONS],
            ('DELETE', '/api/finance/stocks/{symbol}/predictions/{id}'): [Permission.DELETE_PREDICTIONS],
