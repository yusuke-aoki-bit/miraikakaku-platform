...
class FinanceService:
    ...
    async def fetch_comprehensive_stock_data(
        self, symbol: str
        ...
        try:
            ...
            info, history, financials = await asyncio.gather(
    ...
    async def _get_price_history(self, ticker, period: str = "1y") -> pd.DataFrame:
        ...
        return await loop.run_in_executor(
            self.executor, lambda: ticker.history(period=period)
        )
    ...
    async def update_bulk_price_history(
        self, symbols: List[str], period: str = "30d"
    ) -> Dict[str, bool]:
        """複数銘柄の株価履歴を一括更新"""
        results = {}
            ...
            try:
                ...
                for date, row in history.iterrows():
                    ...
                    existing = self.stock_repo.get_price_history(
                        symbol, start_date=date.date(), end_date=date.date()
                    )
