...
def generate_problem_resolution_report():
    ...
    try:
        ...
        logger.info("  解決後:")
        logger.info(f"    - 価格データ: {current_price_stats[1]:,}件 ({current_price_stats[0]}銘柄)")
        ...
        if top_symbols:
            avg_span = sum([row[2] for row in top_symbols if row[2]]) / len([row for row in top_symbols if row[2]])
        ...
        logger.info(f"    - ML適合度: {current_ml_score:.1f}点 (+{improvement:.1f}点改善)")
...
if __name__ == "__main__":
    ...
    logger.info(f"🎉 問題解決完了: ML適合度+{report['ml_score_improvement']:.1f}点向上！")