        ...
        try:
            ...
            for line in lines:
                ...
                if any(word in line for word in ['✅', '成功', 'SUCCESS', '完了']):
                    success_count += 1
                if any(word in line.upper() for word in ['ERROR', 'FAILED', 'EXCEPTION']):
                    error_count += 1
            ...
            if total_target == float('inf'):
                ...
                if file_age_hours > 0:
                    ...
                    if processing_rate > 0:
                        ...
                        estimated_completion_time = datetime.now() + timedelta(hours=remaining_hours)
                        estimated_completion = estimated_completion_time.strftime('%Y-%m-%d %H:%M:%S')
                    else:
                ...
                if elapsed_ratio < 1:
                    remaining_hours = log_info['typical_duration_hours'] - file_age_hours
                    estimated_completion_time = datetime.now() + timedelta(hours=remaining_hours)
                    estimated_completion = estimated_completion_time.strftime('%Y-%m-%d %H:%M:%S')
            ...
            log_analysis[log_info['name']] = {
                ...
                'total_target': total_target if total_target != float('inf') else 'infinite',
    ...
    for name, info in analysis.items():
        ...
        if 'progress_percent' in info and info['progress_percent'] != 'N/A':
            if isinstance(info['progress_percent'], (int, float)):
                logger.info(f"  進捗: {info['progress_percent']:.1f}% ({info['processed_count']}/{info['total_target']})")
        
        logger.info(f"  予想完了: {info['estimated_completion']}")
        
        if info.get('latest_activity'):
            activity = info['latest_activity'][:60] + '...' if len(info['latest_activity']) > 60 else info['latest_activity']
            logger.info(f"  最新活動: {activity}")
        
        logger.info(f"  状態: ✅{info['success_count']}件成功, ❌{info['error_count']}件エラー")
        
        # 完了時刻をリストに追加
        completion_str = info['estimated_completion']
        if completion_str not in ['N/A', '継続実行（無限ループ）', '完了済み', '予定時間超過', '速度不明', '開始直後']:
            try:
    ...
    if completion_times:
        ...
        for name, completion_time in completion_times:
            ...
            if time_until.total_seconds() > 0:
                ...
                logger.info(f"  {completion_time.strftime('%H:%M')} - {name} (あと{hours_until:.1f}時間)")
            else:
                logger.info(f"  {completion_time.strftime('%H:%M')} - {name} (予定時刻経過)")
        ...
        if total_time_until.total_seconds() > 0:
            logger.info("")
            logger.info(f"🎉 全バッチ完了予測: {last_completion.strftime('%Y-%m-%d %H:%M:%S')}")
