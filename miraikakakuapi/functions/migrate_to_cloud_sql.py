...
class DatabaseMigrator:
    ...
    def analyze_data_differences(self):
        ...
        tables_to_check = ['stock_master', 'stock_predictions', 'stock_price_history', 'ai_inference_log']
        ...
        for table in tables_to_check:
            try:
                ...
                logger.info(f"{table}: SQLite={sqlite_count:,}, Cloud SQL={cloud_count:,}")
                
                if sqlite_count > cloud_count:
                    logger.warning(f"⚠️  {table}: SQLiteの方が{sqlite_count - cloud_count:,}レコード多い")
                elif cloud_count > sqlite_count:
                    logger.info(f"ℹ️  {table}: Cloud SQLの方が{cloud_count - sqlite_count:,}レコード多い")
                else:
    ...
    def migrate_table_data(self, table_name, batch_size=1000):
        ...
        try:
            ...
            with self.cloud_sql_engine.connect() as conn:
                existing_result = conn.execute(text(f"SELECT COUNT(*) FROM {table_name}"))
                existing_count = existing_result.fetchone()[0]
                
                if existing_count > 0:
                    logger.warning(f"{table_name}: Cloud SQLに既存データ{existing_count:,}レコードあり")
                    response = input(f"{table_name}の既存データを削除して移行しますか? (y/N): ")
                    if response.lower() == 'y':
            ...
            for i in range(0, len(df), batch_size):
                ...
                # Cloud SQLに挿入
                batch_df.to_sql(table_name, self.cloud_sql_engine, 
                               if_exists='append', index=False, method='multi')
                
                logger.info(f"{table_name}: バッチ{batch_num}/{total_batches}完了 ({len(batch_df)}レコード)")
    ...
    def test_cloud_sql_connection(self):
        ...
        try:
            with self.cloud_sql_engine.connect() as conn:
                ...
                # 主要テーブルのレコード数確認
                for table in ['stock_master', 'stock_predictions', 'stock_price_history']:
                    if table in tables:
    ...
    def run_full_migration(self):
        ...
        # 4. テーブル移行
        tables_to_migrate = ['stock_master', 'stock_predictions', 'stock_price_history', 'ai_inference_log']
        ...
        if success_count == len(tables_to_migrate):
            ...
            logger.warning(f"⚠️  {success_count}/{len(tables_to_migrate)}テーブル移行完了")
