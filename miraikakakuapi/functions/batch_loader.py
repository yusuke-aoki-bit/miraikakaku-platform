...
class BatchDataLoader:
    ...
    def get_target_symbols(self, limit=100):
        ...
        try:
            ...
            logger.info(f"処理対象: 指数{len(indices)}個, 米国株{len(us_stocks[:50])}個, 日本株{len(jp_stocks[:20])}個")
            return all_symbols
            ...
            for date, row in hist.iterrows():
                try:
                    ...
                    """), {
                        "symbol": db_symbol,
                        "date": date.date(),
                        "open_price": float(row['Open']) if row['Open'] > 0 else None,
                        "high_price": float(row['High']) if row['High'] > 0 else None,
                        "low_price": float(row['Low']) if row['Low'] > 0 else None,
                        "close_price": float(row['Close']) if row['Close'] > 0 else None,
                        "volume": int(row['Volume']) if row['Volume'] > 0 else 0,
                        "adjusted_close": float(row['Close']) if row['Close'] > 0 else None,
    ...
    def generate_predictions(self, symbol):
        ...
        try:
            ...
            # 価格変動分析
            if len(prices) >= 2:
                volatility = np.std(prices) / np.mean(prices) if np.mean(prices) > 0 else 0.02
