        ...
        try:
            ...
            if result.returncode == 0:
                ...
                comparison[service] = {
                    ...
                    'port': container.get('ports', [{}])[0].get('containerPort', 'Unknown')
                }
...
def test_api_functionality():
    ...
    services = [
        ...
        ('miraikakaku-api-enhanced', 'https://miraikakaku-api-enhanced-465603676610.us-central1.run.app')
    ]
    ...
    for service_name, base_url in services:
        ...
        for endpoint, description in endpoints:
            try:
                ...
                test_cmd = ['curl', '-s', '-o', '/dev/null', '-w', '%{http_code}', test_url, '-m', '5']
...
def check_traffic_allocation():
    ...
    for service in services:
        try:
            cmd = ['gcloud', 'run', 'services', 'describe', service, '--region=us-central1', '--format=value(spec.traffic[].percent)']
            result = subprocess.run(cmd, capture_output=True, text=True)
