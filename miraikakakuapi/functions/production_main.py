...
async def get_stock_volume(
    symbol: str,
    limit: int = Query(30, ge=1, le=365, description="取得日数")
):
    """株式出来高データ取得API"""
    try:
...
async def get_volume_predictions(
    ...
    try:
        ...
        for i in range(1, days + 1):
            ...
            predicted_volume = max(predicted_volume, int(avg_volume * 0.1))  # 最小値制限
            ...
            predictions.append({
                ...
                "confidence_score": round(confidence, 3),
                "volume_trend": "increasing" if trend_factor > 0 else "decreasing",
            ...
            try:
                ...
                if not hist.empty and len(hist) > 1:
                    ...
                    rankings.append({
                        ...
                        "price_change_percent": round(((hist['Close'].iloc[-1] - hist['Close'].iloc[-2]) / hist['Close'].iloc[-2]) * 100, 2),
...
async def get_sector_details(
    ...
    try:
        ...
        # セクター別銘柄マッピング
        sector_stocks = {
            "technology": ["AAPL", "MSFT", "GOOGL", "NVDA", "META", "CRM", "ADBE", "NFLX", "INTC", "AMD"],
            "financials": ["JPM", "V", "MA", "BAC", "WFC", "C", "GS", "AXP", "USB", "PNC"],
            "healthcare": ["JNJ", "PFE", "UNH", "ABT", "MRK", "TMO", "DHR", "BMY", "AMGN", "GILD"],
            "consumer_discretionary": ["AMZN", "TSLA", "HD", "MCD", "NKE", "LOW", "SBUX", "TJX", "BKNG", "CMG"],
            "industrial": ["BA", "CAT", "GE", "UPS", "HON", "MMM", "LMT", "UNP", "FDX", "RTX"],
            "energy": ["XOM", "CVX", "COP", "SLB", "EOG", "PSX", "VLO", "MPC", "PXD", "OXY"]
        }
...
async def get_theme_details(
    ...
    try:
        ...
        # テーマ別銘柄マッピング
        theme_stocks = {
            "ai_revolution": ["NVDA", "MSFT", "GOOGL", "AAPL", "META", "TSLA", "AMD", "INTC", "CRM", "ADBE"],
            "green_energy": ["TSLA", "ENPH", "SEDG", "PLUG", "NEE", "ICLN", "PBW", "QCLN", "FSLR", "SPWR"],
            "healthcare_innovation": ["MRNA", "GILD", "BIIB", "REGN", "VRTX", "TMO", "DHR", "ISRG", "DXCM", "VEEV"],
            "fintech_disruption": ["PYPL", "SQ", "COIN", "HOOD", "AFRM", "SOFI", "LC", "UPST", "NU", "PAGS"],
            "space_economy": ["SPCE", "RKLB", "ASTR", "PL", "MAXR", "IRDM", "GSAT", "SATS", "LMT", "BA"],
            "cloud_computing": ["AMZN", "MSFT", "CRM", "SNOW", "NET", "DDOG", "ZM", "OKTA", "TWLO", "MDB"]
        }
        ...
        theme_data = {
            "theme_id": theme_name,
            "theme_name": theme_info.get(theme_name, {}).get("name", theme_name.title()),
            "description": theme_info.get(theme_name, {}).get("description", f"{theme_name} related theme"),
            "total_stocks": len(stocks),
            "last_updated": datetime.now().isoformat(),
            "performance": {
            ...
            try:
                ...
                if not hist.empty:
                    ...
                    theme_data["stocks"].append({
                        ...
                        "market_cap": market_cap,
                        "sector": sector,
                        "theme_relevance": round(random.uniform(0.6, 1.0), 2)  # テーマとの関連度
                    })
            except Exception as e:
    ...
    try:
        ...
        for i in range(min(limit, len(templates) * 3)):
            ...
            news_data.append({
                ...
                "source": random.choice(["日経新聞", "ロイター", "Bloomberg", "TechCrunch"]),
                "category": "market_analysis",
                "relevance_score": round(random.uniform(0.7, 1.0), 2),
                "sentiment": random.choice(["positive", "neutral", "negative"]),
                "related_stocks": theme_stocks.get(theme_id, [])[:3] if 'theme_stocks' in locals() else []
            })
...
async def get_theme_ai_insights(
    theme_id: str
):
    """テーマAI分析取得API"""
    try:
...
async def get_currency_history(
    pair: str,
    days: int = Query(30, ge=1, le=365, description="履歴期間")
):
    """通貨履歴取得API"""
    try:
    ...
    try:
        ...
        for i in range(1, limit + 1):
            ...
            predictions.append({
                ...
                "volatility_forecast": "low" if abs(trend_factor) < 0.005 else "medium",
    ...
    try:
        ...
        return {
            ...
            "contest_categories": [
                ...
                {"category": "セクター予測", "count": 14, "avg_accuracy": 66.8}
            ],
            "last_updated": datetime.now().isoformat()
        }
...
async def get_active_contests():
    ...
    try:
        ...
        # 模擬アクティブコンテスト生成
        contest_types = [
            {"type": "stock_prediction", "name": "株価予測", "difficulty": "中級"},
            {"type": "forex_prediction", "name": "為替予測", "difficulty": "上級"},
            {"type": "index_prediction", "name": "指数予測", "difficulty": "初級"},
            {"type": "sector_prediction", "name": "セクター予測", "difficulty": "中級"}
        ]
        ...
        for i in range(5):
            ...
            active_contests.append({
                ...
                "time_remaining": max(0, int((end_date - datetime.now()).total_seconds())),
                "status": "active",
                "rules": {
                    "prediction_range": "±30%",
                    "max_predictions": 1,
                ...
                "leaderboard_preview": [
                    {"rank": 1, "username": "PredictionMaster", "prediction": round(current_price * random.uniform(0.9, 1.1), 2)},
                    {"rank": 2, "username": "MarketGuru", "prediction": round(current_price * random.uniform(0.9, 1.1), 2)},
                    {"rank": 3, "username": "TraderAI", "prediction": round(current_price * random.uniform(0.9, 1.1), 2)}
                ]
            })
...
async def get_past_contests():
    ...
    try:
        ...
        for i in range(20):
            ...
            symbol = random.choice(["AAPL", "TSLA", "NVDA", "MSFT", "GOOGL", "AMZN", "META"])
            ...
            past_contests.append({
                ...
                "winner": {
                    ...
                    "prediction": round(actual_price * random.uniform(0.98, 1.02), 2),
                ...
                "contest_stats": {
                    "avg_prediction": round(target_price * random.uniform(0.95, 1.05), 2),
    ...
    try:
        ...
        for i in range(50):
            ...
            leaderboard.append({
                ...
                "badges": user_badges,
                "recent_performance": [
                    {"contest_id": f"c{j}", "accuracy": round(random.uniform(60, 95), 1)} 
                    for j in range(5)
                ],
                "specialties": random.sample(["株価予測", "為替予測", "指数予測", "セクター予測"], 
                                           random.randint(1, 3)),
        ...
        return {
            ...
            "stats": {
                "avg_accuracy": round(sum(u["accuracy"] for u in leaderboard) / len(leaderboard), 2),
    ...
    try:
        ...
        for i in range(random.randint(50, 200)):
            ...
            participants.append({
                ...
                "user_level": random.choice(["初心者", "中級者", "上級者", "エキスパート"]),
        ...
        return {
            ...
            "your_rank": random.randint(1, len(participants)) if random.choice([True, False]) else None,
            "statistics": {
                "avg_prediction": round(sum(p["prediction"] for p in participants) / len(participants), 2),
                "median_prediction": round(sorted([p["prediction"] for p in participants])[len(participants)//2], 2),
                "prediction_std": round(np.std([p["prediction"] for p in participants]), 2),
                "avg_confidence": round(sum(p["confidence"] for p in participants) / len(participants), 3)
            },
            "last_updated": datetime.now().isoformat()
        }
...
async def submit_prediction(
    ...
    try:
        ...
        # 予測範囲検証
        if prediction <= 0:
            raise HTTPException(status_code=400, detail="予測価格は正数である必要があります")
        
        if confidence < 0 or confidence > 1:
            raise HTTPException(status_code=400, detail="信頼度は0-1の範囲である必要があります")
...
async def get_prediction_factors(prediction_id: str):
    ...
    try:
        ...
        return {
            ...
            "overall_confidence": round(sum(f["confidence"] * f["weight"] for f in ai_factors), 2),
        ...
        raise HTTPException(status_code=500, detail="予測ファクターの取得に失敗しました")
...
async def get_all_ai_factors():
    ...
    try:
        ...
        return {
            ...
            "categories": {
                "technical": len([f for f in all_factors if f["category"] == "technical"]),
                "sentiment": len([f for f in all_factors if f["category"] == "sentiment"]),
                "fundamental": len([f for f in all_factors if f["category"] == "fundamental"]), 
        ...
        raise HTTPException(status_code=500, detail="AIファクター一覧の取得に失敗しました")
...
async def get_symbol_ai_factors(symbol: str, days: int = 30):
    ...
    try:
        ...
        current_price = float(hist['Close'].iloc[-1])
        price_change = ((current_price - float(hist['Close'].iloc[-2])) / float(hist['Close'].iloc[-2])) * 100
        ...
        symbol_factors = [
            ...
            },
            {
                ...
                "factor_name": "出来高パターン",
                "weight": 0.26,
                "impact": "positive" if hist['Volume'].iloc[-1] > hist['Volume'].rolling(10).mean().iloc[-1] else "neutral",
                "confidence": 0.71,
                "current_value": {
                    "volume_ratio": round(float(hist['Volume'].iloc[-1] / hist['Volume'].rolling(10).mean().iloc[-1]), 2),
            ...
            },
            {
                ...
                "confidence": 0.68,
                "current_value": {
                    "volatility_20d": round(float(hist['Close'].pct_change().rolling(20).std() * np.sqrt(252) * 100), 2),
                    "price_range": round(((hist['High'].iloc[-1] - hist['Low'].iloc[-1]) / hist['Close'].iloc[-1]) * 100, 2)
                },
            ...
            },
            {
                ...
                "confidence": 0.64,
                "current_value": {
                    "beta_estimate": round(np.random.uniform(0.8, 1.3), 2),  # Mock beta
        ...
        return {
            ...
            "summary": {
                ...
                "neutral_factors": len([f for f in symbol_factors if f["impact"] == "neutral"]),
                "dominant_factor": max(symbol_factors, key=lambda x: x["weight"])["factor_name"]
            },
        ...
        raise HTTPException(status_code=500, detail=f"銘柄 {symbol} のAIファクター分析に失敗しました")
...
async def get_news(limit: int = 20, category: str = None, symbol: str = None):
    ...
    try:
        ...
        if symbol:
            all_news = [news for news in all_news if symbol.upper() in news["affected_symbols"]]
        ...
        return {
            ...
            "available_categories": ["monetary_policy", "earnings", "automotive", "technology", "market_analysis"],
            "filters_applied": {
                "category": category,
                "symbol": symbol,
                "limit": limit
...
今後の政策パスについて、パウエル議長は「データ次第」との立場を強調し、次回3月会合での利下げ開始の可能性については明言を避けた。市場では6月までに利下げが開始されるとの見方が強まっている。""",
                ...
                "market_reaction": {
                    ...
                    "affected_sectors": ["financials", "technology", "real_estate"],
...
async def get_category_news(category: str, limit: int = 20):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="カテゴリニュース取得に失敗しました")
...
async def get_user_profile(user_id: str = "user_123"):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="ユーザープロフィール取得に失敗しました")
...
async def get_user_watchlist(user_id: str = "user_123"):
    ...
    try:
        ...
        positive_movers = len([item for item in watchlist_items if item["price_change"] > 0])
        negative_movers = len([item for item in watchlist_items if item["price_change"] < 0])
        ...
        return {
            ...
            "summary": {
                ...
                "neutral_movers": len(watchlist_items) - positive_movers - negative_movers
            },
            "timestamp": datetime.now().isoformat(),
...
async def add_to_watchlist(
    ...
    try:
        ...
        if info.empty:
            raise HTTPException(status_code=404, detail=f"銘柄 {symbol} が見つかりません")
    ...
    try:
        ...
        return {
            ...
            "summary": {
                ...
                "combined_gain_loss_percent": round((total_gain_loss / total_cost) * 100, 2) if total_cost > 0 else 0
            },
            "timestamp": datetime.now().isoformat(),
        ...
        raise HTTPException(status_code=500, detail="ポートフォリオ一覧取得に失敗しました")
    ...
    try:
        ...
        portfolio_details = {
            ...
            "summary": {
                ...
                "total_unrealized_gain_loss_percent": round((total_unrealized_gain_loss / total_cost_basis) * 100, 2) if total_cost_basis > 0 else 0,
                "total_day_change": round(total_day_change, 2),
                "total_day_change_percent": round((total_day_change / total_market_value) * 100, 2) if total_market_value > 0 else 0,
                "cash_balance": 2500.00
            },
            "allocation": {
                "by_sector": sector_allocation,
                "by_position": [
                    {"symbol": pos["symbol"], "value": pos["market_value"], "weight": pos["weight"]} 
        ...
        raise HTTPException(status_code=500, detail="ポートフォリオ詳細取得に失敗しました")
...
async def add_transaction(
    ...
    try:
        if transaction_type not in ["buy", "sell"]:
            raise HTTPException(status_code=400, detail="取引タイプは 'buy' または 'sell' である必要があります")
            
        if quantity <= 0 or price <= 0:
            raise HTTPException(status_code=400, detail="数量と価格は正の数である必要があります")
    ...
    try:
        ...
        for i in range(20):
            transaction = {
                ...
                "transaction_date": (datetime.now() - timedelta(days=i*7)).isoformat(),
...
async def get_market_overview():
    ...
    try:
        ...
        positive_indices = len([idx for idx in market_data if idx["daily_change_percent"] > 0])
        negative_indices = len([idx for idx in market_data if idx["daily_change_percent"] < 0])
        
        market_sentiment = "bullish" if positive_indices > negative_indices else "bearish" if negative_indices > positive_indices else "neutral"
        
        # Sector performance (mock data)
        sector_performance = [
            {"sector": "Technology", "change_percent": 1.45, "top_movers": ["AAPL", "MSFT", "GOOGL"]},
            {"sector": "Healthcare", "change_percent": 0.32, "top_movers": ["JNJ", "PFE", "UNH"]},
            {"sector": "Financials", "change_percent": -0.18, "top_movers": ["JPM", "BAC", "WFC"]},
            {"sector": "Energy", "change_percent": 2.15, "top_movers": ["XOM", "CVX", "COP"]},
            {"sector": "Consumer Discretionary", "change_percent": 0.87, "top_movers": ["TSLA", "AMZN", "HD"]}
        ]
        
        # Economic indicators (mock data)
        economic_indicators = {
            "vix": {"value": 18.2, "change": -1.1, "interpretation": "低ボラティリティ"},
            "yield_10y": {"value": 4.15, "change": 0.05, "interpretation": "金利上昇傾向"},
            "dxy": {"value": 103.2, "change": 0.3, "interpretation": "ドル高進行"},
            "oil_wti": {"value": 78.5, "change": 1.2, "interpretation": "原油価格上昇"},
            "gold": {"value": 2034.5, "change": -8.3, "interpretation": "金価格軟調"}
        }
        ...
        return {
            ...
            "market_sentiment": {
                ...
                "neutral_indices": len(market_data) - positive_indices - negative_indices
            },
            "sector_performance": sector_performance,
            "economic_indicators": economic_indicators,
            "market_summary": {
                "risk_appetite": "moderate" if market_sentiment == "neutral" else ("high" if market_sentiment == "bullish" else "low"),
                "volatility_level": "low" if economic_indicators["vix"]["value"] < 20 else ("moderate" if economic_indicators["vix"]["value"] < 30 else "high"),
                "key_themes": ["金利動向", "AI関連銘柄", "エネルギー価格", "地政学リスク"]
            },
            "timestamp": datetime.now().isoformat(),
...
async def get_correlation_matrix(symbols: str, period: str = "1mo"):
    ...
    try:
        ...
        for i, symbol1 in enumerate(symbol_list):
            for j, symbol2 in enumerate(symbol_list):
                ...
                correlations.append({
                    ...
                    "relationship": (
                        "strong_positive" if correlation_value > 0.7 else
                        "moderate_positive" if correlation_value > 0.3 else
                        "weak_positive" if correlation_value > 0.1 else
                        "neutral" if abs(correlation_value) <= 0.1 else
                        "weak_negative" if correlation_value > -0.3 else
                        "moderate_negative" if correlation_value > -0.7 else
                        "strong_negative"
                    )
                })
        ...
        strongest_positive = max(non_diagonal, key=lambda x: x["correlation"] if x["correlation"] < 1 else 0)
        ...
        diversification_score = 1 - abs(avg_correlation)  # Higher score = better diversification
        ...
        return {
            ...
            "analysis": {
                ...
                "diversification_rating": (
                    "excellent" if diversification_score > 0.8 else
                    "good" if diversification_score > 0.6 else
                    "moderate" if diversification_score > 0.4 else
                    "poor"
                )
            },
            "insights": [
