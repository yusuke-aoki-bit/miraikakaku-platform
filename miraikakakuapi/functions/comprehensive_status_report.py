    ...
    for log_file in log_files:
        if os.path.exists(log_file):
            try:
                ...
                # エラー数をカウント
                error_count = sum(
                    1 for line in lines if "ERROR" in line or "エラー" in line
                )
                ...
                batch_status[log_file] = {
                    ...
                    "active": (datetime.now() - last_update).seconds
                    < 300,  # 5分以内に更新
                }
                ...
                logger.info(
                    f"  状態: {
                        '🟢 アクティブ' if batch_status[log_file]['active'] else '🔴 停止中'}"
                )
...
def check_api_status():
    ...
    try:
        ...
        api_status["health"] = {"accessible": False, "error": str(e)}
    ...
    for endpoint in endpoints:
        try:
            ...
            api_status[endpoint] = {"accessible": False, "error": str(e)}
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            batch_processes = [
                ...
                if any(
                    batch in p for batch in ["batch", "expansion", "boost", "pipeline"]
                )
            ]
            ...
            if batch_processes:
                ...
                for proc in batch_processes[:5]:  # 最初の5個のみ表示
                    logger.info(
                        f"    {proc.split()[-1] if proc.split() else 'Unknown'}"
                    )
        ...
        if result.returncode == 0:
            ...
            if len(lines) > 1:
                ...
                logger.info(
                    f"💾 ディスク使用量: {disk_info[2]}/{disk_info[1]} ({disk_info[4]})"
                )
...
def generate_comprehensive_status_report():
    ...
    logger.info(
        f"  価格データ: {
            data_completeness:.2f}% (目標100,000件に対し{
            db_status['price_records']:,        }件)"
    )
    logger.info(
        f"  銘柄カバー率: {
            symbol_coverage:.2f}% ({
            db_status['price_symbols']}/{
                db_status['active_symbols']}銘柄)"
    )
    ...
    total_batches = len(
        [f for f in batch_status.keys() if not batch_status[f].get("missing", False)]
    )
    ...
    logger.info(
        f"  総エラー数: {
            sum(
                status.get(
                    'error_count',
                    0) for status in batch_status.values() if isinstance(
                    status,
                    dict))}"
    )
    ...
    if db_status["orphan_price_symbols"] > 0:
        issues.append(
            f"価格データに{db_status['orphan_price_symbols']}個の外部キー制約違反"
        )
    if db_status["orphan_pred_symbols"] > 0:
        issues.append(
            f"予測データに{db_status['orphan_pred_symbols']}個の外部キー制約違反"
        )
