    ...
    def get_guaranteed_data_symbols(self):
        ...
        return {
            ...
            'mega_cap': [
                ...
                'NFLX', 'ADBE', 'CRM', 'ORCL', 'AVGO', 'AMD', 'INTC', 'QCOM'
            ],
            ...
            # 日本株主要
            'japan': [
                '7203.T', '9984.T', '8306.T', '9983.T', '6098.T', '4063.T',
                '9432.T', '2914.T', '4519.T', '8316.T', '7267.T', '6861.T',
                '4578.T', '6954.T', '8035.T', '9434.T', '4502.T', '8801.T'
            ]
        }
    ...
    def generate_turbo_predictions(self, db, db_symbol, hist_data):
        ...
        try:
            ...
            for days in range(1, 121):
                ...
                # 2. 平均回帰成分
                reversion = -0.1 * (latest_price / sma_50 - 1) * np.sqrt(days / 30) if sma_50 > 0 else 0
                ...
                # 5. 季節性（週・月パターン）
                seasonal = 0.01 * np.sin(2 * np.pi * days / 7) + 0.005 * np.sin(2 * np.pi * days / 30)
                
                # 6. ランダムファクター
                random_shock = np.random.normal(0, 0.005) * np.sqrt(days / 7)
                
                # 総合予測
                total_change = (trend_component + reversion + vol_component + 
                              volume_effect + seasonal + random_shock)
