    ...
    for job in batch_jobs:
        try:
            ...
            cmd = [
                ...
                '--min-instances', '0',
                '--port', '8080',
                '--set-env-vars', f'BATCH_SCRIPT={job["script"]}',
            ...
            if result.returncode == 0:
                ...
                # サービスURLを取得
                url_cmd = [
                    'gcloud', 'run', 'services', 'describe', job['name'],
    ...
    for job in scheduler_jobs:
        try:
            ...
            cmd = [
                ...
                '--uri', f"https://{job['target']}-123456789-uc.a.run.app",  # 実際のURLに置換
                '--http-method', 'POST',
                '--location', region,
                '--description', job['description']
            ]
...
CMD ["python", "docker-entrypoint.py"]'''
    ...
    with open('/mnt/c/Users/yuuku/cursor/miraikakaku/miraikakakuapi/functions/Dockerfile', 'w') as f:
...
'''
    ...
    with open('/mnt/c/Users/yuuku/cursor/miraikakaku/miraikakakuapi/functions/docker-entrypoint.py', 'w') as f:
...
if __name__ == "__main__":
    ...
    logger.info("=" * 80)
    logger.info(f"✅ デプロイ完了: {len(deployed)} サービス, {len(scheduled)} スケジューラー")
