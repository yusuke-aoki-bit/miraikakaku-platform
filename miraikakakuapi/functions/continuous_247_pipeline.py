...
class Continuous247Pipeline:
    ...
    def safe_data_collection(self, symbol):
        ...
        try:
            ...
            try:
                ...
                if exists == 0:
                    return {'symbol': symbol, 'prices': 0, 'predictions': 0, 'error': 'Not in stock_master'}
    ...
    def execute_data_cycle(self):
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            futures = [
                executor.submit(self.safe_data_collection, symbol) 
        ...
        logger.info(f"  今回追加: 価格{cycle_stats['prices']}, 予測{cycle_stats['predictions']}")
        logger.info(f"  累計追加: 価格{self.total_added['prices']}, 予測{self.total_added['predictions']}")
        logger.info(f"  処理成功率: {((cycle_stats['processed']-cycle_stats['errors'])/cycle_stats['processed']*100):.1f}%")
        ...
        while self.running:
            try:
                ...
                if self.cycle_count % 10 == 0:
                    logger.info(f"💗 24/7パイプライン稼働中 - サイクル{self.cycle_count}回完了")
