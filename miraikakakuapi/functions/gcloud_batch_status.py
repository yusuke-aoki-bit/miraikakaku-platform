...
def get_cloud_run_status():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            batch_services = [s for s in services if 'batch' in s.get('metadata', {}).get('name', '').lower()]
            ...
            for service in batch_services:
                ...
                try:
                    ...
                    status = "🟢 正常" if status_code == "200" else f"🔴 異常({status_code})"
                except:
                    status = "⚠️  不明"
...
def get_scheduler_status():
    ...
    try:
        cmd = ['gcloud', 'scheduler', 'jobs', 'list', '--location=us-central1', '--format', 'json']
        result = subprocess.run(cmd, capture_output=True, text=True)
        ...
        if result.returncode == 0:
            ...
            for job in batch_jobs:
                ...
                try:
                    history_cmd = [
                        'gcloud', 'scheduler', 'jobs', 'run-history', name,
                        '--location=us-central1', '--limit=3', '--format=json'
                    ]
                    history_result = subprocess.run(history_cmd, capture_output=True, text=True, timeout=10)
...
def generate_monitoring_report():
    ...
    service_count = len([s for s in services if 'batch' in s.get('metadata', {}).get('name', '').lower()])
    job_count = len(jobs)
    ...
    logger.info(f"  システム健全性: {health_score:.0f}% ({passed_checks}/{total_checks})")
