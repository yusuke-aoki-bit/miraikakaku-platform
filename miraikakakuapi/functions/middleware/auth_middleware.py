        ...
        try:
            ...
            try:
                ...
                if not user or not user.is_active:
                    raise HTTPException(
                        status_code=401, detail="ユーザーが見つからないか無効です"
                    )

                # RBAC権限チェック
                if not rbac_service.can_access_endpoint(
                    user.role, request.url.path, request.method
                ):
                    raise HTTPException(
    ...
    def decorator(func):
        async def wrapper(*args, **kwargs):
            ...
            if not rbac_service.has_permission(user.role, permission):
                raise HTTPException(
                    status_code=403, detail=f"権限が不足しています: {permission.value}"
                )
