    ...
    try:
        ...
        if ADVANCED_FEATURES:
            try:
                ...
                for symbol in symbols:
                    ...
                    if not df.empty:
                        ...
                        try:
                            ...
                            logger.warning(f"Yahoo Finance info failed for {symbol}: {yf_error}")
                            stocks_data.append({
                                "symbol": symbol,
                                "name": symbol,
                                "price": round(latest_price, 2),
                ...
                if stocks_data:
                    result = {"stocks": stocks_data, "count": len(stocks_data), "source": "cloud_sql"}
        ...
        result = {"stocks": stocks_data, "count": len(stocks_data), "source": "yahoo_finance"}
        EnhancedCache.set_cached_data(cache_key, result)
        return result
        ...
        return {
            "stocks": [
                {"symbol": "AAPL", "name": "Apple Inc.", "price": 150.0, "change": 2.5, "sector": "Technology", "data_source": "fallback"},
                {"symbol": "GOOGL", "name": "Alphabet Inc.", "price": 2500.0, "change": -1.2, "sector": "Technology", "data_source": "fallback"},
                {"symbol": "TSLA", "name": "Tesla Inc.", "price": 800.0, "change": 5.3, "sector": "Automotive", "data_source": "fallback"}
            ],
            "count": 3,
            "source": "fallback"
        }
...
async def get_stock_predictions(
    ...
    try:
        ...
        result = {
            ...
            "potential_return": round((predicted_price - current_price) / current_price * 100, 2),
    ...
    try:
        ...
        result = {
            ...
            "portfolio_metrics": {
                ...
                "volatility": round(portfolio_volatility * np.sqrt(252) * 100, 2),  # 年率%
        ...
        raise HTTPException(status_code=500, detail="Failed to perform portfolio analysis")
...
async def get_market_sentiment():
    ...
    try:
        ...
        for index_symbol, index_name in indices.items():
            try:
                ...
                if not hist.empty:
                    ...
                    score = (recent_return * 100) + (month_return * 50) - (volatility * 100)
        ...
        result = {
            ...
            "market_summary": {
                "bullish_indices": sum(1 for data in sentiment_data.values() if data["trend"] == "bullish"),
                "bearish_indices": sum(1 for data in sentiment_data.values() if data["trend"] == "bearish"),
                "neutral_indices": sum(1 for data in sentiment_data.values() if data["trend"] == "neutral")
            },
