...
def generate_final_100_point_report():
    ...
    try:
        ...
        if top_symbols:
            ...
            avg_span = sum([row[4] for row in top_symbols if row[4]]) / len([row for row in top_symbols if row[4]])
        ...
        final_score = data_score + diversity_score + pred_score + time_score + quality_bonus
        ...
        logger.info(f"  総出来高: {price_stats[5]:,}" if price_stats[5] else "  総出来高: N/A")
        
        logger.info(f"\n【予測データ統計】")
        logger.info(f"  銘柄数: {pred_stats[0]:,}")
        logger.info(f"  レコード数: {pred_stats[1]:,}")
        logger.info(f"  期間: {pred_stats[2]} ～ {pred_stats[3]}")
        logger.info(f"  平均信頼度: {pred_stats[4]:.3f}" if pred_stats[4] else "  平均信頼度: N/A")
        logger.info(f"  モデル種類: {pred_stats[5]}")
        
        logger.info(f"\n【今日の活動】")
        logger.info(f"  追加価格データ: {today_price_stats[0]:,}件 ({today_price_stats[1]}銘柄)")
        logger.info(f"  追加予測データ: {today_pred_stats[0]:,}件 ({today_pred_stats[1]}銘柄)")
        
        logger.info(f"\n【データ充実度TOP10】")
        logger.info("Symbol     Records    Span    Period                  Avg Price    Volatility")
        logger.info("-" * 80)
        for symbol, records, start, end, span, avg_price, volatility in top_symbols[:10]:
            vol_str = f"{volatility:.2f}" if volatility else "N/A"
            price_str = f"${avg_price:.2f}" if avg_price else "N/A"
            logger.info(f"{symbol:10} {records:7,} {span:7d} {start} - {end} {price_str:>9} {vol_str:>10}")
        
        logger.info(f"\n【予測モデル統計】")
        logger.info("Model                    Predictions  Symbols  Confidence  Accuracy   Period")
        logger.info("-" * 80)
        for model, preds, symbols, conf, acc, min_date, max_date in model_stats:
            ...
            logger.info(f"{model_short:20} {preds:10,} {symbols:7,} {conf_str:>10} {acc_str:>9} {min_date}~{max_date}")
