...
from .models import (
    ...
    UserProfileCreate,
    UserProfileUpdate,
    AiDecisionFactorResponse,
    EnhancedPredictionResponse,
    WatchlistResponse,
    WatchlistCreate,
    PortfolioResponse,
    PredictionContestResponse,
    ContestPredictionResponse,
    ContestPredictionCreate,
...
async def create_user_profile(
    ...
    try:
        ...
        existing_user = (
            ...
            .filter(UserProfiles.user_id == profile.user_id)
            .first()
        )
        if existing_user:
...
async def get_user_profile(user_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if not user:
            raise HTTPException(
                status_code=404, detail="ユーザープロファイルが見つかりません"
            )
        ...
        raise HTTPException(
            status_code=500, detail="プロファイル取得エラーが発生しました"
        )
...
async def get_ai_decision_factors(
    ...
    try:
        factors = (
            db.query(AiDecisionFactors)
            .filter(AiDecisionFactors.prediction_id == prediction_id)
            .order_by(AiDecisionFactors.influence_score.desc())
            .all()
        )
...
async def get_all_ai_factors(
    ...
    try:
        factors = (
            db.query(AiDecisionFactors)
            .order_by(AiDecisionFactors.created_at.desc())
            .offset(offset)
            .limit(limit)
            .all()
        )
...
@router.get(
    "/ai-factors/symbol/{symbol}", response_model=List[AiDecisionFactorResponse]
)
async def get_symbol_ai_factors(
    symbol: str, limit: int = 50, db: Session = Depends(get_db)
):
    """特定銘柄の全AI決定要因を取得"""
    try:
        # 銘柄の予測IDを取得
        prediction_ids = (
            db.query(StockPredictions.id)
            .filter(StockPredictions.symbol == symbol)
            .subquery()
        )

        factors = (
            db.query(AiDecisionFactors)
            .filter(AiDecisionFactors.prediction_id.in_(prediction_ids))
            .order_by(AiDecisionFactors.influence_score.desc())
            .limit(limit)
            .all()
        )
...
@router.get(
    "/stocks/{symbol}/predictions/enhanced",
    response_model=List[EnhancedPredictionResponse],
)
async def get_enhanced_predictions(symbol: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        for prediction in predictions:
            ...
            factors = (
                db.query(AiDecisionFactors)
                .filter(AiDecisionFactors.prediction_id == prediction.id)
                .order_by(AiDecisionFactors.influence_score.desc())
                .limit(3)
                .all()
            )
            ...
            enhanced_predictions.append(
                EnhancedPredictionResponse(
                    ...
                    decision_factors=[
                        AiDecisionFactorResponse.from_orm(f) for f in factors
                    ],
                )
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"強化予測データ取得エラー: {str(e)}"
        )
...
async def add_to_watchlist(
    user_id: str, watchlist_item: WatchlistCreate, db: Session = Depends(get_db)
):
    """ウォッチリストに銘柄を追加"""
    try:
        # ユーザー存在確認
        ...
        if existing:
            raise HTTPException(
                status_code=400, detail="この銘柄は既にウォッチリストに追加されています"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"ウォッチリスト追加エラー: {str(e)}"
        )
...
async def get_user_watchlist(user_id: str, db: Session = Depends(get_db)):
    ...
    try:
        watchlist = (
            db.query(UserWatchlists)
            .filter(UserWatchlists.user_id == user_id)
            .order_by(UserWatchlists.priority.desc(), UserWatchlists.added_at.desc())
            .all()
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"ウォッチリスト取得エラー: {str(e)}"
        )
...
async def get_user_portfolio(user_id: str, db: Session = Depends(get_db)):
    ...
    try:
        portfolio = (
            db.query(UserPortfolios)
            .filter(UserPortfolios.user_id == user_id, UserPortfolios.is_active is True)
            .all()
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"ポートフォリオ取得エラー: {str(e)}"
        )
...
async def get_active_contests(db: Session = Depends(get_db)):
    ...
    try:
        contests = (
            db.query(PredictionContests)
            .filter(PredictionContests.status == "active")
            .order_by(PredictionContests.prediction_deadline.desc())
            .all()
        )
...
async def submit_contest_prediction(
    ...
    try:
        ...
        contest = (
            db.query(PredictionContests)
            .filter(PredictionContests.id == contest_id)
            .first()
        )
        if not contest:
        ...
        if contest.status != "active":
            raise HTTPException(
                status_code=400, detail="このコンテストは終了しています"
            )

        # 既存予測チェック
        existing = (
            db.query(UserContestPredictions)
            .filter(
        ...
        if existing:
            raise HTTPException(
                status_code=400, detail="このコンテストには既に予測を投稿済みです"
            )

        db_prediction = UserContestPredictions(
            contest_id=contest_id, **prediction.dict()
        )
...
async def get_theme_insights(
    ...
    try:
        insights = (
            db.query(ThemeInsights)
            .filter(ThemeInsights.theme_name == theme_name)
            .order_by(ThemeInsights.insight_date.desc())
            .limit(limit)
            .all()
        )
...
async def get_all_insights(
    ...
    try:
        insights = (
            db.query(ThemeInsights)
            .order_by(ThemeInsights.created_at.desc())
            .offset(offset)
            .limit(limit)
            .all()
        )
...
async def get_latest_insights(
    ...
    try:
        ...
        insights = (
            db.query(ThemeInsights)
            .filter(ThemeInsights.created_at >= cutoff_date)
            .order_by(ThemeInsights.impact_score.desc())
            .limit(limit)
            .all()
        )
...
@router.get(
    "/insights/categories/{category}", response_model=List[ThemeInsightResponse]
)
async def get_category_insights(
    ...
    try:
        insights = (
            db.query(ThemeInsights)
            .filter(ThemeInsights.theme_category == category)
            .order_by(ThemeInsights.insight_date.desc())
            .limit(limit)
            .all()
        )
