...
async def get_stock_data_from_db(db: Session, symbol: str, days: int):
    ...
    try:
        # Try stock_prices table (actual DB table name with correct column names)
        query = text("""
...
async def get_stock_price(
    ...
    if yf_data:
        logger.info(f"Found {len(yf_data)} records from Yahoo Finance for {mapped_symbol}")
        
        # Optionally save to database for future use
        try:
            for record in yf_data[:10]:  # Save last 10 records as sample
                insert_query = text("""
                    INSERT IGNORE INTO stock_prices (symbol, date, open_price, high_price, low_price, close_price, volume)
...
async def get_stock_predictions(
    ...
    try:
        pred_query = text("""
            SELECT prediction_date, predicted_price, confidence_score, model_version
        ...
        start_date = datetime.now() - timedelta(days=days)
        result = db.execute(pred_query, {"symbol": symbol.upper(), "start_date": start_date})
            ...
            for i in range(min(7, len(actual_data))):
                if i < len(actual_data):
                    ...
                    historical_predictions.append({
                        ...
                        "value": data_point["close"] * (1 + np.random.normal(0, 0.02)),
...
async def get_database_stats(db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Count by market
        result = db.execute(text("SELECT market, COUNT(*) as count FROM stock_master GROUP BY market"))
        stats["by_market"] = {row[0]: row[1] for row in result}
        
        # Count by country
        result = db.execute(text("SELECT country, COUNT(*) as count FROM stock_master GROUP BY country"))
