...
from api.models.admin_models import (
    ...
    SystemStatsResponse, ModelPerformanceResponse
)
from services.auth_service import AuthService
from services.rbac_service import Permission
from middleware.auth_middleware import get_current_user, require_permission, require_role
...
async def get_system_stats(
    ...
    try:
        ...
        total_stocks = db.query(StockMaster).filter(StockMaster.is_active == True).count()
        total_predictions = db.query(StockPredictions).count()
        
        # 今日の活動
        today = datetime.utcnow().date()
        today_predictions = db.query(StockPredictions).filter(
            StockPredictions.prediction_date >= today
        ).count()
        
        today_inferences = db.query(AIInferenceLog).filter(
            AIInferenceLog.created_at >= today
        ).count()
...
async def restart_system(request: Request):
    ...
    return {"message": "システム再起動を開始しました", "requested_by": current_user.email}
...
async def get_recent_logs(
    ...
    """最近のシステムログ取得"""
    try:
        logs = db.query(AIInferenceLog).order_by(
            AIInferenceLog.created_at.desc()
        ).limit(limit).all()
