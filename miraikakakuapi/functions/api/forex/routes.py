...
import pandas as pd
import logging
from config import ForexConfig, FinanceConfig
...
async def get_currency_pairs():
    ...
    try:
        ...
        return {"status": "success", "data": pairs, "count": len(pairs)}
...
async def get_currency_rate(pair: str):
    ...
    try:
        ...
        if normalized_pair not in MAJOR_CURRENCY_PAIRS:
            raise HTTPException(
                status_code=404, detail="指定された通貨ペアは対応していません"
            )
        ...
        if hist.empty:
            raise HTTPException(
                status_code=404, detail="レートデータが取得できませんでした"
            )
        ...
        change_percent = (
            (change / previous_rate) * FinanceConfig.PERCENTAGE_MULTIPLIER
            if previous_rate != 0
            else 0
        )

        # Bid/Ask スプレッドの推定（実際のデータではないため推定値）
        spread = latest_rate * ForexConfig.DEFAULT_SPREAD_PERCENTAGE
...
async def get_currency_history(
    pair: str,
    days: int = Query(
        ForexConfig.DEFAULT_HISTORY_DAYS,
        ge=1,
        le=ForexConfig.MAX_HISTORY_DAYS,
        description="取得する日数",
    ),
):
    """通貨ペアの履歴データを取得"""
    try:
        ...
        if hist.empty:
            raise HTTPException(
                status_code=404, detail="履歴データが取得できませんでした"
            )
...
async def get_currency_predictions(
    ...
    limit: int = Query(
        ForexConfig.DEFAULT_PREDICTION_LIMIT,
        ge=1,
        le=ForexConfig.MAX_PREDICTION_LIMIT,
        description="予測データ数",
    ),
):
    """通貨ペアの予測データを生成（簡易実装）"""
    try:
        ...
        time_increments = {
            ...
            "1M": timedelta(days=ForexConfig.MONTH_DAYS),
        }
        ...
        predictions = []
        base_volatility = current_rate * ForexConfig.BASE_VOLATILITY_PERCENTAGE

        for i in range(1, limit + 1):
            future_time = datetime.now() + (increment * i)

            # 簡易的なトレンド計算（実際にはより複雑な予測モデルを使用）
            trend_factor = ForexConfig.TREND_FACTOR_BASE * (i % 3 - 1)
            # 時間と共にボラティリティ増加
            volatility = base_volatility * (
                ForexConfig.VOLATILITY_BASE_FACTOR
                + (i * ForexConfig.VOLATILITY_INCREASE_RATE)
            )
            ...
            confidence = max(
                ...
                ForexConfig.PREDICTION_CONFIDENCE_BASE
                - (i * ForexConfig.PREDICTION_CONFIDENCE_DECAY),
            )
    ...
    try:
        ...
        if country:
            economic_events = [
                ...
                if event["country"] == country.upper()
            ]
