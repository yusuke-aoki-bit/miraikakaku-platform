    ...
    for log_file in log_files:
        try:
            if os.path.exists(log_file):
                ...
                log_status.append({"file": log_file, "exists": False})

        except Exception as e:
            log_status.append({"file": log_file, "error": str(e), "exists": False})
...
def generate_batch_status_report():
    ...
    for log in active_logs:
        ...
        status_emoji = (
            ...
            else "🟡" if age.total_seconds() < 3600 else "🔴"
        )

        logger.info(
            f"{status_emoji} {
                log['file']}: {
                log['size_mb']:.1f}MB, 更新{age_str}"
        )
    ...
    if db_activity.get("db_accessible", False):
        ...
        logger.info(
            f"📊 直近1時間: 価格{
                db_activity['1時間の価格追加']}件, 予測{
                db_activity['1時間の予測追加']}件"
        )
        logger.info(f"🎯 対象銘柄: {db_activity['1時間の対象銘柄']}銘柄")
    else:
        logger.error(
            f"❌ データベース接続エラー: {db_activity.get('error', 'Unknown')}"
        )
    ...
    if len(processes) == 0:
        logger.warning(
            "⚠️ バッチプロセスが停止中 - restart_batch_processes.py実行を推奨"
        )
