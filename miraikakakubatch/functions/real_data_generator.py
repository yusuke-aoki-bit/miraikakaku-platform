...
class RealTimeAnalyzer:
    ...
    async def get_technical_analysis(self, symbol: str, price_data: List[Dict]) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "trend_strength": (
                    float(trend_strength) if not np.isnan(trend_strength) else 0.0
                ),
                "volume_ratio": (
                    float(volume_ratio) if not np.isnan(volume_ratio) else 1.0
                ),
    ...
    def _calculate_support_resistance(self, prices: pd.Series) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "position_in_range": (current_price - recent_low)
    ...
    def generate_decision_factors(
        self, symbol: str, technical: Dict, fundamental: Dict, prediction_data: Dict
    ) -> List[Dict]:
        """実データに基づくAI判断根拠生成"""
        factors = []
...
def generate_real_ai_decision_factors():
    ...
    try:
        ...
        logger.info(
            f"Processing {len(predictions)} predictions for AI decision factors..."
        )
        ...
        for prediction in predictions:
            try:
                ...
                # テクニカル・ファンダメンタル分析実行
                technical = await analyzer.get_technical_analysis(
                    prediction.symbol, price_dict_list
                )
                ...
                logger.error(
                    f"Error processing prediction {prediction.id} for {prediction.symbol}: {e}"
                )
                continue
