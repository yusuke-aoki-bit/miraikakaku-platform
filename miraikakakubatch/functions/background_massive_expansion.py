...
def background_massive_expansion():
    ...
    with db.engine.connect() as conn:
        # 銘柄数確認
        total = conn.execute(text('SELECT COUNT(*) FROM stock_master WHERE is_active = 1')).scalar()
        ...
        for offset in range(0, total, batch_size):
            ...
            for symbol, country in symbols:
                try:
                    ...
                    if existing_prices < target_prices:
                        ...
                        batch_prices += self.generate_price_data(conn, symbol, need_prices)
                    
                    # 予測データ生成
                    if existing_preds < target_preds:
                        need_preds = min(target_preds - existing_preds, 30)
                        batch_predictions += self.generate_prediction_data(conn, symbol, need_preds)
            ...
            logger.info(f'📦 バッチ完了 {offset+1}-{min(offset+batch_size, total)}: '
            ...
            if processed % 1000 == 0 or processed == total:
                ...
                logger.info(f'📊 全体進捗: {processed:,}/{total:,} ({processed/total*100:.1f}%) - '
    ...
    with open('expansion_complete.txt', 'w') as f:
        f.write(f'completed,{processed},{prices_added},{predictions_added},{estimated_fill_rate:.1f}\n')
    ...
    for days in range(1, count + 1):
        ...
        try:
            ...
            '''), {
                ...
                'p': round(predicted_price, 2), 'conf': round(confidence, 3),
                'days': days, 'm': 'BACKGROUND_MASSIVE_V1', 'a': round(accuracy, 3)
            })
            added += 1
        except:
            continue
