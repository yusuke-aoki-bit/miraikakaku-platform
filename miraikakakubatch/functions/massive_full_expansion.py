...
class MassiveFullExpansion:
    ...
    def smart_symbol_prioritization(self, symbols):
        ...
        tier1_patterns = [
            ...
            'JPM', 'BAC', 'WFC', 'MS', 'C', 'V', 'MA', 'AXP',
            'WMT', 'HD', 'PG', 'JNJ', 'KO', 'PEP', 'MCD', 'NKE', 'DIS',
            'XOM', 'CVX', 'COP', 'BA', 'CAT', 'GE', 'MMM', 'LMT',
            # 指数・ETF
        ...
        tier2_patterns = [
            # 米国株（3-4文字シンボル）
            lambda s: s[2] in ['US', 'USA', 'United States'] and len(s[0]) <= 4 and s[0].isalpha(),
            # 日本株主要（数字.T形式）
        ...
        for symbol_data in symbols:
            ...
            elif any(pattern(symbol_data) if callable(pattern) else symbol == pattern 
                    for pattern in tier2_patterns if callable(pattern)):
    ...
    def bulk_data_generation(self, symbol_data, batch_size=50):
        ...
        try:
            ...
            with db.engine.connect() as conn:
                # 既存データチェック（効率化）
                existing_price_count = conn.execute(text(
                    "SELECT COUNT(*) FROM stock_prices WHERE symbol = :sym"
                ), {"sym": symbol}).scalar()
                
                existing_pred_count = conn.execute(text(
    ...
    def execute_massive_expansion(self, max_workers=8, progress_interval=50):
            ...
            for future in as_completed(future_to_symbol):
                ...
                try:
                    ...
                    if self.total_processed % progress_interval == 0:
                        ...
                        eta_seconds = (total_symbols - self.total_processed) / rate if rate > 0 else 0
                        eta_hours = eta_seconds / 3600
                        
                        logger.info(f"📊 進捗: {self.total_processed:,}/{total_symbols:,} "
        ...
        # 予想充足率計算
        estimated_fill_rate = min(80, (self.total_prices_added + self.total_predictions_added) / (total_symbols * 100) * 100)
