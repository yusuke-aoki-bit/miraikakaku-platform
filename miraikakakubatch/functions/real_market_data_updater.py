...
class RealMarketDataUpdater:
    ...
    def _update_stock_data(self, connection, db_symbol, yahoo_symbol):
        ...
        try:
            ...
            hist = yf.download(yahoo_symbol, start=start_date, end=end_date, progress=False)
            ...
            try:
                ...
                info = ticker.info
                company_name = info.get('longName', info.get('shortName', f"{db_symbol} Corporation"))
            ...
            with connection.cursor() as cursor:
                ...
                for date_idx in hist.index:
                    try:
                        ...
                        if len(hist.columns.levels) > 1:
                            ...
                            adj_close_val = hist.loc[date_idx, 'Adj Close'] if 'Adj Close' in hist.columns else close_val
                        ...
                        """, (
                            ...
                            float(adj_close_val) if not pd.isna(adj_close_val) else None,
                        ...
                        logger.error(f"価格データ挿入エラー {db_symbol} {date_idx}: {e}")
                ...
                if not hist.empty:
                    ...
                    logger.info(f"✅ {db_symbol}: {insert_count}件更新 (最新価格: ¥{latest_price:,.2f})")
                else:
    ...
    def update_us_major_stocks(self):
        ...
        us_major_stocks = [
            ...
            'UBER', 'ZOOM', 'ROKU', 'SHOP', 'CRWD', 'OKTA', 'SNOW', 'DDOG'
        ]
    ...
    def verify_updates(self):
        ...
        try:
            with connection.cursor(pymysql.cursors.DictCursor) as cursor:
                ...
                if result:
                    logger.info(f"✅ 1321(日経225ETF): ¥{result['close_price']:,.2f} ({result['date']})")
                else:
                ...
                if result:
                    logger.info(f"✅ 1306(TOPIX ETF): ¥{result['close_price']:,.2f} ({result['date']})")
                ...
                for row in results:
                    logger.info(f"✅ {row['symbol']}({row['name']}): ¥{row['close_price']:,.2f}")
