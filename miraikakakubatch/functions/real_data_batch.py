...
class RealTimeAnalyzer:
    ...
    def generate_decision_factors(self, symbol: str, technical: Dict, fundamental: Dict) -> List[Dict]:
        """実データに基づくAI判断根拠生成"""
        factors = []
        ...
        for prediction in result:
            try:
                ...
                price_result = db.execute(text("""
                    ...
                    ORDER BY date DESC 
                    LIMIT 50
                """), {"symbol": prediction.symbol}).fetchall()
                ...
                # テクニカル・ファンダメンタル分析実行
                technical = analyzer.get_technical_analysis(prediction.symbol, price_data)
                fundamental = analyzer.get_fundamental_analysis(prediction.symbol)
                
                # AI判断根拠生成
                factors = analyzer.generate_decision_factors(prediction.symbol, technical, fundamental)
...
{"成長期待が継続" if trend == "bullish" else "慎重な投資判断が必要" if trend == "bearish" else "横ばい推移が予想"}されます。
                ...
                """), {"theme_name": theme_name, "insight_date": insight_date}).fetchone()
