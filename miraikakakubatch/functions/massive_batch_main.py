...
class MassiveBatchProcessor:
    ...
    def generate_synthetic_data(self, symbol_info):
        ...
        try:
            with db.engine.connect() as conn:
                # 既存データ確認
                existing_prices = conn.execute(text(
                    'SELECT COUNT(*) FROM stock_prices WHERE symbol = :s'
                ), {'s': symbol}).scalar()
                
                existing_preds = conn.execute(text(
                ...
                if existing_preds < target_preds:
                    ...
                    predictions_added = self._create_prediction_data(conn, symbol, need_preds)
                
                # AI決定要因データ生成（新規追加）
                if predictions_added > 0:
                    factors_added = self._create_ai_factors(conn, symbol, min(predictions_added * 2, 10))
    ...
    def _create_prediction_data(self, conn, symbol, count):
        ...
        for days in range(1, count + 1):
            try:
                ...
                # 4. 平均回帰
                mean_reversion = -0.05 * np.tanh(days / 30) * np.random.uniform(0.5, 1.5)
                
                # 総合変化率
                total_change = (base_trend * days + vol_effect + mean_reversion) * time_decay
                
                predicted_price = current_price * (1 + total_change)
                predicted_price = max(0.01, predicted_price)
                
                # 信頼度計算
                confidence = max(0.2, 0.92 - days * 0.006 + np.random.uniform(-0.05, 0.05))
                ...
                '''), {
                    ...
                    'p': round(predicted_price, 4), 'conf': round(confidence, 3),
                    'days': days, 'm': 'CLOUD_MASSIVE_V1', 'a': round(accuracy, 3)
                })
                added += 1
            except Exception:
                continue
    ...
    def _create_ai_factors(self, conn, symbol, count):
        ...
        # 最新の予測データを取得して関連付け
        try:
            latest_prediction = conn.execute(text('''
        ...
        factor_templates = [
            ...
            ("technical", "移動平均線分析", "短期・長期移動平均線の交差分析", 0.55, 0.75),
            ...
            ("sentiment", "市場センチメント", "投資家心理指標による市場動向", 0.40, 0.65),
    ...
    def run_massive_batch(self):
        ...
        logger.info(f"🎯 処理速度: {self.stats['processed']/(duration/3600):.0f}銘柄/時間")
        ...
        for future in futures:
            ...
            with self.lock:
                ...
                if self.stats['processed'] % 500 == 0:
                    ...
                    eta = (total - self.stats['processed']) / rate / 3600 if rate > 0 else 0
                    
                    logger.info(f"📊 進捗: {self.stats['processed']:,}/{total:,} "
    ...
    def do_GET(self):
        if self.path == '/health':
            ...
            response = {
                ...
                "processing_rate": round(stats['processed'] / elapsed, 1) if elapsed > 0 else 0
            }
