...
def check_database_stats():
    ...
    try:
        ...
        with db.engine.connect() as conn:
            # 銘柄数
            result = conn.execute(
                text("SELECT COUNT(*) as count FROM stock_master")
            ).fetchone()
            ...
            # 価格データ数
            result = conn.execute(
                text("SELECT COUNT(*) as count FROM stock_prices")
            ).fetchone()
            ...
            # 予測データ数
            result = conn.execute(
                text("SELECT COUNT(*) as count FROM stock_predictions")
            ).fetchone()
            ...
            # AI決定要因数
            result = conn.execute(
                text("SELECT COUNT(*) as count FROM ai_decision_factors")
            ).fetchone()
            ...
            # テーマ洞察数
            result = conn.execute(
                text("SELECT COUNT(*) as count FROM theme_insights")
            ).fetchone()
            ...
            # 最新予測データ
            result = conn.execute(
                text("SELECT MAX(prediction_date) FROM stock_predictions")
            ).fetchone()
            ...
            if stock_count > 0:
                ...
                # 1銘柄あたりの平均予測データ数
                avg_predictions = (
                    prediction_count / stock_count if stock_count > 0 else 0
                )
                print(f"🔮 1銘柄あたり平均予測データ数: {avg_predictions:.1f}")

                # 推定データ充足率 (目標: 1銘柄あたり1000価格データ、100予測データ)
                price_fill_rate = (
                    min(100, (avg_prices / 1000) * 100) if avg_prices > 0 else 0
                )
                prediction_fill_rate = (
                    min(100, (avg_predictions / 100) * 100)
                    if avg_predictions > 0
                    else 0
                )
                ...
                print(
                    f"🎯 総合充足率: {(price_fill_rate + prediction_fill_rate) / 2:.1f}%"
                )
