...
def insert_ai_factors_directly():
    ...
    with db.engine.connect() as conn:
        ...
        for pred_id, symbol in result:
            # 各予測に5個の要因を追加
            for i in range(5):
                factor_types = ['technical', 'fundamental', 'sentiment', 'pattern', 'news']
                factor_type = random.choice(factor_types)
                
                factor_names = {
                    'technical': ['RSI分析', '移動平均線', 'MACD', 'ボリューム分析', 'トレンド分析'],
                    'fundamental': ['PER評価', 'ROE分析', '売上成長', '利益率', '財務健全性'],
                    'sentiment': ['市場心理', '投資家動向', 'SNS分析', 'ニュース感情', '機関投資家'],
                    'pattern': ['チャートパターン', 'サポート&レジスタンス', 'トレンドライン', 'フィボナッチ', 'エリオット波動'],
                    'news': ['決算発表', '業界ニュース', 'マクロ経済', '規制変更', '企業イベント']
                }
...
def insert_theme_insights_directly():
    ...
    with db.engine.connect() as conn:
        for theme_name, category in themes:
            # 各テーマに100個の洞察を生成
            for i in range(100):
                insight_date = datetime.now().date() - timedelta(days=random.randint(0, 90))
                
                key_drivers = f"{theme_name}セクターの成長要因{i+1}: 市場拡大、技術革新、規制緩和"
                affected_stocks = ", ".join(random.sample(
