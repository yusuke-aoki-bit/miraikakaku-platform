...
class OptimizedMassiveExpansion:
    ...
    def fast_data_generation(self, symbol_data):
        ...
        try:
            ...
            with db.engine.connect() as conn:
                ...
                existing_predictions = conn.execute(text(
                    "SELECT COUNT(*) FROM stock_predictions WHERE symbol = :sym"
                ), {"sym": symbol}).scalar()
                
                # 既に十分なデータがある場合はスキップ
                if existing_prices > 200 and existing_predictions > 20:
                    return {'symbol': symbol, 'status': 'skip', 'prices': 0, 'predictions': 0}
            ...
            if hist.empty or len(hist) < 5:
                return {'symbol': symbol, 'status': 'no_data', 'prices': 0, 'predictions': 0}
    ...
    def _fast_predictions(self, conn, symbol, hist_data):
        ...
        try:
            ...
            # 簡単な統計
            volatility = np.std(np.diff(prices)) / np.mean(prices) if len(prices) > 1 else 0.02
    ...
    def execute_optimized_massive_expansion(self, batch_size=1000, max_workers=4):
        ...
        with db.engine.connect() as conn:
            total_symbols = conn.execute(text(
                "SELECT COUNT(*) FROM stock_master WHERE is_active = 1"
            )).scalar()
        ...
        while offset < total_symbols:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                future_to_symbol = {
                    executor.submit(self.fast_data_generation, symbol_data): symbol_data[0]
                ...
                for future in as_completed(future_to_symbol, timeout=60):
                    try:
                        ...
                        if total_processed % 100 == 0:
                            ...
                            eta = (total_symbols - total_processed) / rate / 3600 if rate > 0 else 0
                            
                            logger.info(f"📊 進捗: {total_processed:,}/{total_symbols:,} "
        ...
        estimated_fill_rate = min(90, 3.3 + (total_new_data / (total_symbols * 50)) * 100)
