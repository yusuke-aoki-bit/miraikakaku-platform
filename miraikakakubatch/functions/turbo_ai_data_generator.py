...
class TurboAIDataGenerator:
    def __init__(self):
        ...
        self.factor_templates = [
            ...
            (
                "technical",
                "移動平均線クロス",
                "短期MA({short}日)と長期MA({long}日)の{cross_type}",
                0.60,
                0.80,
            ),
            (
            ...
            (
                "fundamental",
                "配当利回り",
                "配当利回り{yield:.2f}%は{attractiveness}",
                0.60,
                0.80,
            ),
            (
            ...
            (
                "sentiment",
                "市場心理指標",
                "Fear&Greed指数{value}で{sentiment}",
                0.45,
                0.65,
            ),
        ...
        self.theme_templates = [
            (
                "Technology",
                "tech",
                ["AI進化", "量子コンピューティング", "5G展開", "サイバーセキュリティ"],
            ),
            (
                "Healthcare",
                "health",
                ["バイオテクノロジー", "遠隔医療", "個別化医療", "医療AI"],
            ),
            (
                "Energy",
                "energy",
                ["再生可能エネルギー", "水素経済", "電池技術", "炭素中立"],
            ),
            ("Finance", "finance", ["デジタル通貨", "DeFi", "ネオバンク", "ESG投資"]),
            (
                "Consumer",
                "consumer",
                ["D2C", "サブスクリプション", "体験経済", "サステナブル消費"],
            ),
            (
                "Industrial",
                "industrial",
                ["自動化", "IoT", "3Dプリンティング", "スマートファクトリー"],
            ),
            (
                "Materials",
                "materials",
                ["新素材", "リサイクル技術", "ナノテクノロジー", "バイオマテリアル"],
            ),
            (
                "Real Estate",
                "realestate",
                ["PropTech", "スマートシティ", "グリーンビルディング", "共有経済"],
            ),
            (
                "Transportation",
                "transport",
                ["EV革命", "自動運転", "空飛ぶ車", "ハイパーループ"],
            ),
            (
                "Communication",
                "telecom",
                ["メタバース", "AR/VR", "衛星通信", "エッジコンピューティング"],
            ),
        ]
            ...
            with db.engine.begin() as conn:
                for pred_id in prediction_ids:
                    ...
                    selected_templates = random.sample(
                        self.factor_templates, num_factors
                    )

                    for template in selected_templates:
                        factor_type, name_template, desc_template, min_inf, max_inf = (
                            template
                        )
    ...
    def _generate_factor_values(self, factor_type):
        ...
        if factor_type == "technical":
            ...
            values["pattern_name"] = random.choice(
                [
                    "三角保ち合い",
                    "ダブルトップ",
                    "ダブルボトム",
            ...
            values["harmonic"] = random.choice(
                ["バット", "ガートレー", "バタフライ", "クラブ"]
            )
            values["candle_pattern"] = random.choice(
                ["ハンマー", "包み線", "明けの明星", "三兵"]
            )
            ...
            values["sector"] = random.choice(
                ["テクノロジー", "ヘルスケア", "エネルギー", "金融"]
            )
            values["event"] = random.choice(
                ["規制緩和", "新技術導入", "業界再編", "需要増加"]
            )
            values["indicator"] = random.choice(["GDP", "CPI", "雇用統計", "金利"])
            values["change"] = random.choice(
                ["上昇", "低下", "予想上回る", "予想下回る"]
            )
            values["regulation"] = random.choice(
                ["環境規制", "データ保護", "金融規制", "独占禁止"]
            )
    ...
    def _generate_key_drivers(self, category, subtopic):
        ...
        base_drivers = drivers_templates.get(
            category, [f"{subtopic}の市場動向が活発化"]
        )
    ...
    def run_turbo_generation(self):
            ...
            for future in as_completed(futures):
                try:
                    ...
                    if completed % 10 == 0:
                        ...
                        logger.info(
                            f"進捗: {completed}/{len(futures)} バッチ完了 - "
        ...
        logger.info(
            f"🚀 処理速度: {(self.stats['ai_factors_added'] + self.stats['theme_insights_added'])/duration:.1f}件/秒"
        )
        logger.info("=" * 80)
