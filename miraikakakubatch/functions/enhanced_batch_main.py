...
class EnhancedStockDataProcessor:
    ...
    async def train_lstm_models(self, symbols: List[str]) -> Dict[str, Any]:
        ...
        for symbol in symbols:
            try:
                ...
                training_results[symbol] = {
                    ...
                    "data_points": len(df),
                    "final_loss": (
                        history.get("loss", [])[-1] if history.get("loss") else None
                    ),
                    "validation_loss": (
        ...
        if self.trained_symbols:
            task_status["lstm_model_status"] = (
                f"trained_for_{len(self.trained_symbols)}_symbols"
            )
    ...
    async def generate_predictions(self, symbols: List[str]) -> Dict[str, Any]:
        ...
        if predictions and self.db_repo:
            try:
                predictions_list = [
                    {**pred, "symbol": symbol} for symbol, pred in predictions.items()
                ]
            ...
            try:
                ...
                predictions[symbol] = {
                    ...
                    "potential_return": float(
                        (predicted_prices[-1] - base_price) / base_price * 100
                    ),
    ...
    async def generate_comprehensive_report(
        ...
        try:
            report = self.report_generator.generate_comprehensive_report(
                predictions=predictions, market_data=market_data
            )
            ...
            if self.db_repo:
                try:
                    report_summary = {
                        ...
                        "market_sentiment": report.get("market_overview", {}).get(
                            "sentiment", "neutral"
                        ),
                        "top_performers": [
                            rec["symbol"]
                        ...
                        "key_insights": report.get("executive_summary", {}).get(
                            "key_insights", []
                        ),
                    }
    ...
    async def _fallback_report(self, predictions: Dict) -> Dict[str, Any]:
        ...
        avg_return = (
            np.mean([p.get("potential_return", 0) for p in predictions.values()])
            if predictions
            else 0
        )
...
class BatchTasks:
    ...
    async def update_stock_prices():
        ...
        try:
            ...
            logger.info(
                f"Updated {len(result['data'])} stocks, stored {result['stored_records']} records"
            )
            return {
                "status": "success",
    ...
    async def train_lstm_models():
        ...
        try:
            ...
            logger.info(
                f"LSTM training completed: {successful_training}/{len(symbols)} models trained"
            )
    ...
    async def generate_comprehensive_report():
        ...
        try:
            ...
            # レポート生成
            report = await stock_processor.generate_comprehensive_report(
...
async def run_batch_task(task_name: str):
    ...
    if db_repository:
        try:
            log_id = db_repository.insert_batch_log(
                batch_type=task_name, status="started"
            )
        except Exception as e:
    ...
    try:
        ...
        if log_id and db_repository:
            try:
                db_repository.update_batch_log(
                    log_id=log_id, status="failed", error_message=str(e)
                )
            except Exception as e:
                logger.error(f"Failed to update batch log: {e}")
...
def schedule_tasks():
    ...
    schedule.every().sunday.at("02:00").do(
        lambda: asyncio.create_task(run_batch_task("train_models"))
    )

    # 毎日15時に予測生成
    schedule.every().day.at("15:00").do(
        lambda: asyncio.create_task(run_batch_task("generate_predictions"))
    )
...
async def get_detailed_status():
    ...
    return {
        ...
        "system_info": {
            ...
            "trained_symbols": (
                len(stock_processor.trained_symbols)
                if stock_processor.trained_symbols
                else 0
            ),
        },
    }
...
async def startup_event():
    ...
    logger.info(
        "Enhanced batch system is ready with LSTM and advanced reporting capabilities"
    )
...
async def get_database_status():
    ...
    try:
        ...
        return {"status": "error", "message": str(e), "stocks": 0}
