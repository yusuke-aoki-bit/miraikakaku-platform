    ...
    try:
        ...
        for prediction in predictions:
            ...
            for factor_template in selected_factors:
                # スコアにランダム性を追加
                influence_score = factor_template["base_influence"] + random.uniform(
                    -0.1, 0.1
                )
                confidence = factor_template["base_confidence"] + random.uniform(
                    -0.1, 0.1
                )
...
async def generate_theme_insights():
    ...
    try:
        ...
        for insight_data in insights_data:
            insight_data["insight_date"] = date.today() - timedelta(
