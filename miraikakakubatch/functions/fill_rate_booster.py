...
def boost_fill_rates():
    ...
    stats = {"prices_added": 0, "predictions_added": 0, "factors_added": 0}

    try:
        with db.engine.connect() as conn:
            # データが少ない銘柄を優先的に取得
            symbols_low_data = conn.execute(
                text(
                    """
                ...
                try:
                    ...
                    if current_prices < 50:
                        ...
                        for i in range(need_prices):
                            date = (
                                datetime.now() - timedelta(days=need_prices - i)
                            ).date()
                    ...
                    total_processed = (
                        stats["prices_added"]
                        + stats["predictions_added"]
                        + stats["factors_added"]
                    )
                    if total_processed > 0 and total_processed % 1000 == 0:
                        logger.info(
                            f"進捗: 価格+{stats['prices_added']}, 予測+{stats['predictions_added']}, 要因+{stats['factors_added']}"
                        )
