...
class VolumePredictionService:
    ...
    def analyze_volume_patterns(
        self, volumes: List[int], dates: List[datetime]
    ) -> Dict:
        """出来高パターンを分析"""
        ...
        if len(volumes) >= 10:
            ...
            older_avg = (
                np.mean(volumes_array[-10:-5]) if len(volumes) >= 10 else mean_volume
            )
    ...
    def generate_volume_prediction(
        self, symbol: str, db: Session, days_ahead: int = 7
    ) -> int:
        """個別銘柄の出来高予測を生成"""
        try:
            # 過去出来高データを取得
            ...
            if "error" in analysis:
                logger.warning(
                    f"Volume analysis failed for {symbol}: {analysis['error']}"
                )
                return 0
            ...
            for i in range(1, days_ahead + 1):
                ...
                # 既存予測をチェック
                existing = (
                    db.query(StockVolumePredictions)
                    .filter(
                ...
                if weekday in analysis["weekday_averages"]:
                    ...
                    weekday_factor = (
                        weekday_avg / analysis["mean_volume"]
                        if analysis["mean_volume"] != 0
                        else 1.0
                    )
            ...
            db.commit()
            logger.info(
                f"Created {predictions_created} volume predictions for {symbol}"
            )
        ...
        try:
            ...
            predictions = (
                ...
                .filter(
                    ...
                    StockVolumePredictions.actual_volume.is_(
                        None
                    ),  # まだ更新されていない
                )
                .all()
            ...
            for prediction in predictions:
                ...
                if actual_data and actual_data.volume:
                    ...
                    if actual_volume != 0:
                        accuracy = (
                            1 - abs(predicted_volume - actual_volume) / actual_volume
                        )
            ...
            if updated_count > 0:
                db.commit()
                logger.info(
                    f"Updated accuracy for {updated_count} volume predictions for {symbol}"
                )
    ...
    def batch_generate_volume_predictions(
        self, limit: int = 100, days_ahead: int = 7
    ) -> Dict[str, int]:
        """複数銘柄の出来高予測を並列生成"""
        results = {
            "predictions_created": 0,
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=4) as executor:
                future_to_symbol = {
                    executor.submit(
                        self._process_single_symbol, symbol, days_ahead
                    ): symbol
                ...
                for future in as_completed(future_to_symbol):
                    ...
                    try:
                        ...
                        if result["predictions_created"] > 0:
                            logger.info(
                                f"Volume predictions for {symbol}: {result['predictions_created']} created"
                            )

                    except Exception as e:
                        logger.error(
                            f"Error processing volume predictions for {symbol}: {str(e)}"
                        )
                        results["errors"] += 1
    ...
    def _process_single_symbol(self, symbol: str, days_ahead: int) -> Dict[str, int]:
        ...
        try:
            with get_db_session() as db:
                # 予測生成
                result["predictions_created"] = self.generate_volume_prediction(
    ...
    def get_volume_prediction_stats(self) -> Dict:
        ...
        try:
            with get_db_session() as db:
                ...
                accuracy_calculated = (
                    db.query(StockVolumePredictions)
                    .filter(StockVolumePredictions.accuracy_score.isnot(None))
                    .count()
                )

                # 平均精度
                avg_accuracy = (
                    db.query(StockVolumePredictions.accuracy_score)
                    .filter(StockVolumePredictions.accuracy_score.isnot(None))
                    .scalar()
                )
                ...
                # 銘柄数
                symbols_count = (
                    db.query(StockVolumePredictions.symbol).distinct().count()
                )
                ...
                return {
                    ...
                    "average_accuracy": (
                        round(avg_accuracy * 100, 2) if avg_accuracy else 0
                    ),
                    "symbols_with_predictions": symbols_count,
                    "accuracy_coverage": (
                        round((accuracy_calculated / total_predictions * 100), 2)
