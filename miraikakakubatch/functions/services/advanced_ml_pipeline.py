...
class AdvancedMLPipeline:
        ...
        try:
            ...
            for i in range(7):  # 7日間の予測
                ...
                last_sequence = np.append(
                    last_sequence[:, 1:, :], new_row.reshape(1, 1, 5), axis=1
                )

            # 逆正規化
            future_prices = self.scaler.inverse_transform(
                np.column_stack(
                    [future_predictions] + [np.zeros((len(future_predictions), 4))]
                )
            )[:, 0]
    ...
    def train_prophet_model(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            df = pd.DataFrame(
                [
                    {"ds": price.date, "y": float(price.close_price)}
                    for price in price_data
                ]
            )
            ...
            df_cv = cross_validation(
                model, initial="365 days", period="30 days", horizon="7 days"
            )
    ...
    def ensemble_prediction(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            prophet_preds = [
                p["yhat"]
                for p in prophet_results["future_predictions"][: len(lstm_preds)]
            ]

            for i in range(min(len(lstm_preds), len(prophet_preds))):
                ensemble_price = (
                    lstm_preds[i] * lstm_weight + prophet_preds[i] * prophet_weight
                )
    ...
    def evaluate_model_performance(
        ...
        try:
            ...
            # 対応する予測を取得
            predictions = (
                self.db_session.query(StockPredictions)
                .filter(
                    ...
                    for pred in model_preds:
                        actual = next(
                            (
                                ...
                                if p.date == pred.target_date.date()
                            ),
                            None,
                        )
                        if actual:
                            matched_pairs.append(
                                (float(pred.predicted_price), float(actual.close_price))
                            )
