...
from models.forex_models import (
    ForexPairs,
    ForexRates,
    ForexPredictions,
    ForexVolumePredictions,
)
...
class ForexDataService:
    ...
    def initialize_forex_pairs(self, db: Session):
        ...
        try:
            for pair_code, info in self.major_pairs.items():
                existing = (
                    db.query(ForexPairs)
                    .filter(ForexPairs.pair_code == pair_code)
                    .first()
                )
    ...
    def fetch_forex_data(
        self, pair_code: str, days: int = 365
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                if existing:
                    ...
                    existing.change_rate = (
                        float(row["change_rate"]) if pd.notna(row["change_rate"]) else 0
                    )
                else:
                    # 新規データ作成
                    rate = ForexRates(
    ...
    def generate_rate_predictions(
        self, pair_code: str, db: Session, days_ahead: int = 7
    ) -> int:
        """為替レート予測を生成"""
        try:
            # 過去データを取得
            historical_data = (
                db.query(ForexRates)
                .filter(ForexRates.pair_code == pair_code)
                .order_by(ForexRates.date.desc())
                .limit(30)
                .all()
            )
            ...
            db.commit()
            logger.info(
                f"Created {predictions_created} rate predictions for {pair_code}"
            )
    ...
    def generate_volume_predictions(
        self, pair_code: str, db: Session, days_ahead: int = 7
    ) -> int:
        """為替出来高予測を生成"""
        try:
            ...
            historical_data = (
                db.query(ForexRates)
                .filter(ForexRates.pair_code == pair_code, ForexRates.volume > 0)
                .order_by(ForexRates.date.desc())
                .limit(30)
                .all()
            )
            ...
            for i in range(1, days_ahead + 1):
                ...
                # 既存予測をチェック
                existing = (
                    db.query(ForexVolumePredictions)
                    .filter(
            ...
            db.commit()
            logger.info(
                f"Created {predictions_created} volume predictions for {pair_code}"
            )
            return predictions_created

        except Exception as e:
            logger.error(
                f"Error generating volume predictions for {pair_code}: {str(e)}"
            )
            db.rollback()
            return 0

    def batch_process_all_pairs(
        self, days_back: int = 365, prediction_days: int = 7
    ) -> Dict[str, int]:
        """全通貨ペアを並列処理"""
        results = {
            "rates_saved": 0,
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=4) as executor:
                future_to_pair = {
                    executor.submit(
                        self._process_single_pair, pair_code, days_back, prediction_days
                    ): pair_code
    ...
    def _process_single_pair(
        self, pair_code: str, days_back: int, prediction_days: int
    ) -> Dict[str, int]:
        """単一通貨ペアの処理"""
        ...
        try:
            with get_db_session() as db:
                ...
                # レート予測生成
                result["rate_predictions"] = self.generate_rate_predictions(
                    pair_code, db, prediction_days
                )

                # 出来高予測生成
                result["volume_predictions"] = self.generate_volume_predictions(
                    pair_code, db, prediction_days
                )
