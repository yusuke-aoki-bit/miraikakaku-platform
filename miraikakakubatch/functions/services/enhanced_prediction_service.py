...
class EnhancedPredictionService:
    ...
    def __init__(self):
        self.model_versions = ['STATISTICAL_V2', 'TREND_FOLLOWING_V1', 'MEAN_REVERSION_V1', 'ENSEMBLE_V1']

    def calculate_technical_indicators(self, prices: List[float], volumes: List[int] = None) -> Dict:
        """テクニカル指標を計算"""
        if len(prices) < 20:
            return {}
        ...
        if len(prices) >= 10:
            ...
            indicators['volatility'] = np.std(returns) * np.sqrt(252)  # 年率ボラティリティ
        ...
        if len(prices) >= 20:
            ...
            indicators['bollinger_position'] = (prices_array[-1] - indicators['bollinger_lower']) / (indicators['bollinger_upper'] - indicators['bollinger_lower'])
    ...
    def generate_multiple_predictions(self, symbol: str, db: Session, days_ahead: int = 30) -> Dict[str, int]:
        """複数モデルによる株価予測を生成"""
        ...
        try:
            ...
            ).order_by(StockPriceHistory.date.desc()).limit(100).all()
            ...
            volumes = [int(h.volume) if h.volume else 0 for h in reversed(historical_data)]
    ...
    def _generate_predictions_with_model(self, symbol: str, db: Session, prices: List[float], 
                                       indicators: Dict, model_version: str, days_ahead: int) -> int:
        """特定モデルでの予測生成"""
        try:
            ...
            for i in range(1, days_ahead + 1):
                ...
                # 既存予測をチェック
                existing = db.query(StockPredictions).filter(
                    and_(
            ...
            if predictions_created > 0:
                db.commit()
                logger.info(f"Created {predictions_created} predictions for {symbol} using {model_version}")
    ...
    def _calculate_prediction_by_model(self, current_price: float, prices: List[float], 
                                     indicators: Dict, model_version: str, days_ahead: int) -> Optional[Tuple[float, float, Dict]]:
        """モデル別の予測計算"""
        try:
            if model_version == 'STATISTICAL_V2':
                return self._statistical_model_v2(current_price, prices, indicators, days_ahead)
            elif model_version == 'TREND_FOLLOWING_V1':
                return self._trend_following_model(current_price, prices, indicators, days_ahead)
            elif model_version == 'MEAN_REVERSION_V1':
                return self._mean_reversion_model(current_price, prices, indicators, days_ahead)
            elif model_version == 'ENSEMBLE_V1':
                return self._ensemble_model(current_price, prices, indicators, days_ahead)
            else:
                return None
    ...
    def _statistical_model_v2(self, current_price: float, prices: List[float], 
                            indicators: Dict, days_ahead: int) -> Tuple[float, float, Dict]:
        """統計モデル V2"""
        # 価格変動率の統計分析
    ...
    def _trend_following_model(self, current_price: float, prices: List[float], 
                             indicators: Dict, days_ahead: int) -> Tuple[float, float, Dict]:
        """トレンドフォローイングモデル"""
        # 移動平均によるトレンド判定
    ...
    def _mean_reversion_model(self, current_price: float, prices: List[float], 
                            indicators: Dict, days_ahead: int) -> Tuple[float, float, Dict]:
        """平均回帰モデル"""
        # 長期平均価格
    ...
    def _ensemble_model(self, current_price: float, prices: List[float], 
                       indicators: Dict, days_ahead: int) -> Tuple[float, float, Dict]:
        """アンサンブルモデル（複数モデルの組み合わせ）"""
        # 各モデルの予測を取得
        statistical = self._statistical_model_v2(current_price, prices, indicators, days_ahead)
        trend = self._trend_following_model(current_price, prices, indicators, days_ahead)
        reversion = self._mean_reversion_model(current_price, prices, indicators, days_ahead)
        ...
        # 重み付き予測価格
        weighted_price = sum(model[0] * weight for model, weight in zip(models, weights)) / total_weight
        
        # アンサンブル信頼度（個別モデル信頼度の調和平均）
        ensemble_confidence = len(weights) / sum(1/w if w > 0 else float('inf') for w in weights)
    ...
    def update_historical_accuracy(self, symbol: str, db: Session) -> int:
        ...
        try:
            ...
            if updated_count > 0:
                db.commit()
                logger.info(f"Updated accuracy for {updated_count} predictions for {symbol}")
    ...
    def batch_generate_enhanced_predictions(self, limit: int = 50, days_ahead: int = 30) -> Dict:
        """強化された予測の一括生成"""
        results = {
            'symbols_processed': 0,
            'total_predictions': 0,
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=6) as executor:
                future_to_symbol = {
                    executor.submit(self._process_single_symbol_enhanced, symbol, days_ahead): symbol
                ...
                for future in as_completed(future_to_symbol):
                    ...
                    try:
                        ...
                        if sum(result['model_results'].values()) > 0:
                            logger.info(f"Enhanced predictions for {symbol}: {result['model_results']}")
                        
                    except Exception as e:
                        logger.error(f"Error processing enhanced predictions for {symbol}: {str(e)}")
                        results['errors'] += 1
    ...
    def _process_single_symbol_enhanced(self, symbol: str, days_ahead: int) -> Dict:
        ...
        try:
            with get_db_session() as db:
                # 複数モデル予測生成
                model_results = self.generate_multiple_predictions(symbol, db, days_ahead)
