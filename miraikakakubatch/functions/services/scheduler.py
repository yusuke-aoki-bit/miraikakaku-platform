...
class MLPipelineScheduler:
        ...
        try:
            ...
            for symbol in active_symbols:
                try:
                    ...
                    logger.info(f"日次予測完了: {symbol} -> ${ensemble_result.get('predicted_price', 0):.2f}")
    ...
    def _run_daily_predictions(self):
        ...
        try:
            ...
            logger.info(f"日次予測完了: Vertex AI {vertex_results.get('successful', 0)} 件成功")
    ...
    def _run_weekly_maintenance(self):
        ...
        try:
            ...
            for model_name in ['lstm_v1', 'prophet_v1', 'vertex_ai_automl']:
                performance = self.vertex_ai_service.monitor_model_performance(model_name, days=7)
            ...
            logger.info(f"週次メンテナンス完了: {cleaned_count} 個のモデルクリーンアップ")
    ...
    def _run_monthly_vertex_ai_training(self):
        ...
        try:
            ...
            if dataset_path:
                # AutoMLモデル作成
                model_resource = self.vertex_ai_service.create_automl_tabular_model(dataset_path)
                
                if model_resource:
                    # モデルをエンドポイントにデプロイ
                    endpoint = self.vertex_ai_service.deploy_model_to_endpoint(model_resource)
    ...
    def _get_active_symbols(self) -> list:
        """アクティブな銘柄リストを取得"""
        try:
            stocks = self.db_session.query(StockMaster).filter(
        ...
        try:
            report = {
                ...
                "summary": {
                    ...
                    "avg_accuracy": sum(p.get('avg_accuracy', 0) for p in performance_results) / len(performance_results) if performance_results else 0,
                    "total_predictions": sum(p.get('total_predictions', 0) for p in performance_results)
                }
            }
