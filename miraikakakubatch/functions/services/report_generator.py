...
class StockReportGenerator:
    ...
    def generate_comprehensive_report(
        ...
        # 3. 個別銘柄分析
        report["individual_analysis"] = self._analyze_individual_stocks(
            predictions, market_data
        )
        ...
        # 5. リスク評価
        report["risk_assessment"] = self._assess_portfolio_risk(
            predictions, market_data
        )

        # 6. 投資推奨
        report["recommendations"] = self._generate_investment_recommendations(
            predictions
        )
    ...
    def _generate_executive_summary(self, predictions: Dict, market_data: Dict) -> Dict:
        ...
        bullish_count = sum(
            1 for p in predictions.values() if p.get("potential_return", 0) > 2
        )
        bearish_count = sum(
            1 for p in predictions.values() if p.get("potential_return", 0) < -2
        )
        ...
        avg_return = np.mean(
            [p.get("potential_return", 0) for p in predictions.values()]
        )
        ...
        top_performer = max(
            predictions.items(), key=lambda x: x[1].get("potential_return", 0)
        )
        worst_performer = min(
            predictions.items(), key=lambda x: x[1].get("potential_return", 0)
        )
        ...
        return {
            ...
            "market_outlook": self._determine_market_outlook(
                avg_return, bullish_count, total_stocks
            ),
            "top_pick": {
        ...
        if market_data:
            ...
            if all_volatilities:
                market_analysis["volatility_analysis"] = {
                    ...
                    "volatility_trend": (
                        "increasing" if np.mean(all_volatilities) > 0.02 else "stable"
                    ),
                    "risk_environment": self._assess_risk_environment(
                        np.mean(all_volatilities)
                    ),
                }
    ...
    def _analyze_individual_stocks(
        self, predictions: Dict, market_data: Dict
    ) -> List[Dict]:
        """個別銘柄分析"""
        analyses = []
        ...
        for symbol, prediction in predictions.items():
            analysis = {
                ...
                "investment_recommendation": self._generate_stock_recommendation(
                    prediction
                ),
        ...
        # リターン順でソート
        analyses.sort(
            key=lambda x: x["current_analysis"]["potential_return"], reverse=True
        )
        return analyses
        ...
        return {
            ...
            "prediction_categories": {
                "strong_buy": [
                    ...
                    if p.get("potential_return", 0) > 5
                ],
                "buy": [
                    s
                    for s, p in predictions.items()
                    if 2 < p.get("potential_return", 0) <= 5
                ],
                "hold": [
                    s
                    for s, p in predictions.items()
                    if -2 <= p.get("potential_return", 0) <= 2
                ],
                "sell": [
                    s
                    for s, p in predictions.items()
                    if p.get("potential_return", 0) < -2
                ],
            },
        }
    ...
    def _generate_investment_recommendations(self, predictions: Dict) -> List[Dict]:
        ...
        top_picks = sorted(
            ...
            key=lambda x: x[1].get("potential_return", 0)
            * (x[1].get("confidence_score", 0) / 100),
            reverse=True,
        )[:3]
        ...
        # 注意銘柄（高リスク）
        high_risk_stocks = [
            s for s, p in predictions.items() if p.get("risk_level") == "high"
        ]
        if high_risk_stocks:
            recommendations.append(
                {
    ...
    def _determine_market_outlook(
        self, avg_return: float, bullish_count: int, total_stocks: int
    ) -> str:
        """市場見通し決定"""
    ...
    def _assess_diversification(self, predictions: Dict) -> Dict:
        ...
        return {
            ...
            "diversification_score": min(
                100, (1 - np.std(returns) / np.mean(np.abs(returns))) * 100
            ),
        }
    ...
    def _generate_risk_recommendations(self, predictions: Dict) -> List[str]:
        ...
        high_risk_count = sum(
            1 for p in predictions.values() if p.get("risk_level") == "high"
        )
        if high_risk_count > len(predictions) * 0.3:
            recommendations.append(
                "ポートフォリオのリスク集中度が高いため、分散投資を検討してください"
            )

        low_confidence_count = sum(
            1 for p in predictions.values() if p.get("confidence_score", 0) < 60
        )
        if low_confidence_count > len(predictions) * 0.4:
            recommendations.append(
                "予測信頼度が低い銘柄が多いため、ポジションサイズを調整してください"
            )
