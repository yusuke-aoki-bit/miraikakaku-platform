...
class VertexAIService:
    ...
    def create_training_dataset(self, symbols: List[str], lookback_days: int = 365) -> Optional[str]:
    ...
    def create_automl_tabular_model(self, dataset_path: str, target_column: str = "target") -> Optional[str]:
    ...
    def predict_with_vertex_ai(self, endpoint_name: str, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if predictions.predictions:
                ...
                confidence = float(predictions.predictions[0][1]) if len(predictions.predictions[0]) > 1 else 0.8
                ...
                logger.info(f"Vertex AI 予測完了: {symbol}, 価格: ${predicted_price:.2f}")
    ...
    def _prepare_prediction_features(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            # 最新50日のデータを取得
            recent_prices = self.db_session.query(StockPriceHistory).filter(
                StockPriceHistory.symbol == symbol
            ).order_by(StockPriceHistory.date.desc()).limit(50).all()
    ...
    def run_batch_prediction(self, symbols: List[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"バッチ予測完了: {len([r for r in results.values() if r])} 件成功")
    ...
    def monitor_model_performance(self, model_name: str, days: int = 30) -> Dict[str, Any]:
        """モデル性能を監視"""
        try:
            start_date = datetime.utcnow() - timedelta(days=days)
            
            # 推論ログを取得
            logs = self.db_session.query(AIInferenceLog).filter(
            ...
            avg_inference_time = np.mean([log.inference_time_ms for log in logs if log.inference_time_ms])
            avg_confidence = np.mean([log.confidence_score for log in logs if log.confidence_score])
            
            # エラー率
            error_rate = (total_inferences - successful_inferences) / total_inferences * 100
