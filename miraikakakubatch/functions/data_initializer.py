...
class CloudSQLInitializer:
    ...
    def initialize_stock_master_data(self):
        ...
        try:
            ...
            logger.info(
                f"株式マスターデータ初期化開始: {len(COMPREHENSIVE_JAPANESE_STOCKS)}社"
            )

            # 既存の日本株データを削除
            result = session.execute(
                text("DELETE FROM stock_master WHERE country = 'Japan'")
            )
            ...
            for symbol, info in COMPREHENSIVE_JAPANESE_STOCKS.items():
                ...
                if len(batch_values) >= batch_size:
                    ...
                    logger.info(
                        f"投入進捗: {total_inserted}/{len(COMPREHENSIVE_JAPANESE_STOCKS)}"
                    )
                    batch_values = []
            ...
            if grand_total == len(COMPREHENSIVE_JAPANESE_STOCKS):
                ...
                logger.error(
                    f"データ不整合: 期待{len(COMPREHENSIVE_JAPANESE_STOCKS)}社 実際{grand_total}社"
                )
                return False
    ...
    def get_initialization_status(self):
        ...
        try:
            ...
            return {"status": "error", "message": str(e), "japanese_stocks": 0}
