        ...
        for symbol, data in stock_data.items():
            try:
                ...
                predictions[symbol] = {
                    ...
                    "potential_return": round((predicted_price - base_price) / base_price * 100, 2),
...
async def run_batch_task(task_name: str):
    ...
    try:
        ...
        task_status["running_tasks"].remove(task_name) if task_name in task_status["running_tasks"] else None
...
def schedule_tasks():
    ...
    # 毎日深夜2時にクリーンアップ
    schedule.every().day.at("02:00").do(lambda: asyncio.create_task(run_batch_task("cleanup")))
